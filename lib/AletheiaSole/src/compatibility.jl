...
function Base.show(io::IO, ::_UnsupportedName{Name}) where {Name}
    return print(io, "unsupported SoleLogics.", Name)
end
function (value::_UnsupportedName{Name})(args...) where {Name}
    return _unsupported(Name, "this legacy value is a deliberate compatibility gap")
end
_unsupported_name(name::Symbol) = _UnsupportedName{name}()

function _unsupported(name::Symbol, detail::AbstractString)
    return throw(
        ArgumentError("SoleLogics.$name has no faithful Aletheia equivalent: $detail")
    )
end
...
const _DEFAULT_SIGNATURE = Aletheia.Signature((
...
const _AnyConnective = Union{
    NamedConnective,
    Aletheia.Negation,
    Aletheia.Conjunction,
    Aletheia.Disjunction,
    Aletheia.Implication,
    Aletheia.AbstractRelationalConnective,
}
function _named_connective(::Val{name}) where {name}
    return _unsupported(:NamedConnective, "Aletheia has no connective spelled $(name)")
end
...
const _LegacyConnective = Union{
    Aletheia.Negation,Aletheia.Conjunction,Aletheia.Disjunction,Aletheia.Implication
}
...
function truths(formula::Aletheia.Formula)
    return [
        node isa Truth ? node : value(node) for
        node in formulas(formula) if node isa Truth ||
        ((node isa Atom || node isa Aletheia.Atom) && value(node) isa Truth)
    ]
end
truths(value::Truth) = Truth[value]
function collatetruth(args...)
    return _unsupported(
        :collatetruth,
...
Base.getindex(children::_CompatChildren, i::Int) = _wrap_id(children.pool, children.ids[i])
Base.iterate(::_CompatChildren{0}) = nothing
@inline function Base.iterate(children::_CompatChildren{N}) where {N}
    return N == 0 ? nothing : (_wrap_id(children.pool, children.ids[1]), 2)
end
@inline function Base.iterate(children::_CompatChildren{N}, state::Int) where {N}
    state > N && return nothing
    return (_wrap_id(children.pool, children.ids[state]), state + 1)
end
Base.IteratorSize(::_CompatChildren) = Base.HasLength()
Base.IteratorEltype(::_CompatChildren) = Base.EltypeUnknown()
function Base.:(==)(children::_CompatChildren, values::Tuple)
    return length(children) == length(values) &&
           all(children[i] == values[i] for i in eachindex(values))
end
...
struct Atom{V,P<:Aletheia.FormulaPool} <: Aletheia.Formula
    ...
    id::Int
    payload::V
    function Atom{V,P}(pool::P, id::Int, payload::V) where {V,P<:Aletheia.FormulaPool}
        return new{V,P}(pool, id, payload)
    end
    function Atom(
        native::Aletheia.Atom{V,P}, ::Val{:native}
    ) where {V,P<:Aletheia.FormulaPool}
        return new{V,P}(native.pool, native.id, native.value)
    end
end
struct _CompatBranch{C,N,P<:Aletheia.FormulaPool} <: Aletheia.Formula
    ...
    function _CompatBranch{C,N,P}(
        pool::P, id::Int, connective::C, ids::NTuple{N,Int}
    ) where {C,N,P<:Aletheia.FormulaPool}
        return new{C,N,P}(pool, id, connective, _CompatChildren{N,P}(pool, ids))
    end
    function _CompatBranch(
        native::Aletheia.Branch{C,N,P}
    ) where {C,N,P<:Aletheia.FormulaPool}
        return _CompatBranch{C,N,P}(
            native.pool, native.id, native.connective, native.children
        )
    end
end
...
@inline function _wrap(formula::Aletheia.Atom{V,P}) where {V,P<:Aletheia.FormulaPool}
    return _wrap_id(formula.pool, formula.id)
end
@inline function _wrap(formula::Aletheia.Branch{C,N,P}) where {C,N,P<:Aletheia.FormulaPool}
    return _wrap_id(formula.pool, formula.id)
end
@inline _unwrap(formula::Atom) = Aletheia._formula_unlocked(formula.pool, formula.id)
@inline function _unwrap(formula::_CompatBranch)
    return Aletheia._formula_unlocked(formula.pool, formula.id)
end
...
function Base.:(==)(left::_CompatBranch, right::_CompatBranch)
    return left.pool === right.pool && left.id == right.id
end
...
function Base.hash(formula::_CompatBranch, h::UInt)
    return hash(objectid(formula.pool), hash(formula.id, h))
end
@inline function _wrap_id(pool::P, id::Int) where {P<:Aletheia.FormulaPool}
    ...
    try
        ...
        cached === nothing || return cached
        node = pool.nodes[id]
        wrapped = if node.kind == 0x01
            (
                if node.payload isa Truth
                    node.payload
                else
            ...
            _CompatBranch{typeof(node.payload),length(node.children),P}(
                pool, id, node.payload, node.children
            )
        end
        cache[id] = wrapped
        wrapped
...
function Atom(value)
    value isa Truth &&
        _unsupported(:Atom, "truth values are semantic values, not formulas in Aletheia")
    value isa Aletheia.Formula && _unsupported(
        :Atom, "Aletheia atoms cannot contain formulas; use children/branch instead"
    )
    return _wrap(Aletheia.atom(_DEFAULT_POOL, value))
end
...
function (connective::Aletheia.Negation)(formula::Aletheia.Formula)
    return Branch(connective, formula)
end
function (connective::Union{Aletheia.Conjunction,Aletheia.Disjunction,Aletheia.Implication})(
    left::Aletheia.Formula, right::Aletheia.Formula
)
    return Branch(connective, left, right)
end
...
function _formula_pool_for(connective, formulas::Tuple)
    ...
    return _merge_formula_pools(connective, formulas)
end
...
function _repool(formula::Truth, target)
    return Aletheia.atom(target, formula)
end
function _repool(formula::Aletheia.Formula, target)
    Aletheia.pool(formula) === target && return _wrap(formula)
    formula isa Aletheia.Atom &&
        return _wrap(Aletheia.atom(target, Aletheia.value(formula)))
    formula isa Aletheia.Branch || _unsupported(
        :SyntaxBranch, "children must be Aletheia formulas (got $(typeof(formula)))"
    )
    return _wrap(
        Aletheia.branch(
            target,
            Aletheia.operator(formula),
...
@inline function _compat_child_id(child::Aletheia.Atom, pool)
    return Aletheia.pool(child) === pool ? Aletheia.id(child) : 0
end
@inline function _compat_child_id(child::Aletheia.Branch, pool)
    return Aletheia.pool(child) === pool ? Aletheia.id(child) : 0
end
...
@inline function _compat_branch(connective, child::F) where {F<:_CompatFormula}
    ...
    pool = child.pool
    _hasconnective(Aletheia.signature(pool), connective) ||
        return _compat_branch(connective, (child,))
    connective isa Aletheia.Negation && return _compat_negation(pool, child.id)
    return _wrap_id(pool, Aletheia._intern!(pool, 0x02, connective, (child.id,)))
end
@inline function _compat_branch(
    connective, left::F, right::G
) where {F<:_CompatFormula,G<:_CompatFormula}
    ...
    if right.pool === pool && _hasconnective(Aletheia.signature(pool), connective)
        return _wrap_id(
            pool, Aletheia._intern!(pool, 0x02, connective, (left.id, right.id))
        )
    end
    return _compat_branch(connective, (left, right))
end
...
function _compat_branch(connective, children::Tuple)
    ...
    expected == length(children) || throw(
        ArgumentError(
    ...
    return _wrap(
        Aletheia.branch(
            pool, connective, ntuple(i -> _unwrap(normalized[i]), length(normalized))
        ),
    )
end
function Branch(connective::_LegacyConnective, children::Tuple)
    connective isa _UnsupportedName && _unsupported(
        :SyntaxBranch, "the requested connective is not implemented by Aletheia"
    )
    return _compat_branch(connective, children)
end
function Branch(connective::_LegacyConnective, children::Tuple{F}) where {F<:_CompatFormula}
    return if Aletheia.arity(connective) == 1
        _compat_branch(connective, children[1])
    else
        _compat_branch(connective, children)
    end
end
function Branch(
    connective::_LegacyConnective, children::Tuple{F,G}
) where {F<:_CompatFormula,G<:_CompatFormula}
    return if Aletheia.arity(connective) == 2
        _compat_branch(connective, children[1], children[2])
    else
        _compat_branch(connective, children)
    end
end
...
function formulas(formula::Aletheia.Formula)
    return _walk(formula, Aletheia.Formula[])
end
...
function subformulas(formula::Union{Atom,_CompatBranch}; sorted=true)
    return sorted ? _cached_subformulas(formula) : formulas(formula)
end
function subformulas(formula::Aletheia.Formula; sorted=true)
    result = formulas(formula)
    return sorted ? sort!(result; by=height) : result
end
...
function connectives(formula::Aletheia.Formula)
    return [
        token(node) for
        node in formulas(formula) if node isa _CompatBranch || node isa Aletheia.Branch
    ]
end
...
function _flatten(formula::Aletheia.Formula, connective)
    return _flatten!(Aletheia.Formula[], formula, connective)
end
...
function branch(pool::Aletheia.FormulaPool, connective, children...)
    return _wrap(Aletheia.branch(pool, connective, Tuple(_unwrap.(children))))
end
branch(connective::_LegacyConnective, children...) = Branch(connective, children...)
...
function syntaxstring(formula::Aletheia.Formula; kwargs...)
    ...
    isempty(unknown) || _unsupported(
        :syntaxstring,
        "Aletheia's printer does not accept keyword(s) $(join(string.(unknown), ", ")).",
    )
    return Aletheia.syntaxstring(_unwrap(formula))
end
function syntaxstring(connective::_CompatConnective; kwargs...)
    return Aletheia.notation(connective.native)
end
...
function _payload(atom_parser, text)
    ...
    parsed isa Truth &&
        _unsupported(:parseformula, "truth values are not formula leaves in Aletheia")
    return if parsed isa Atom
        value(parsed)
    elseif parsed isa Aletheia.Atom
        Aletheia.value(parsed)
    else
        parsed
    end
end
function _parse_pool(additional_operators)
    ...
    return Aletheia.FormulaPool(
        Aletheia.Signature((Aletheia.:¬, Aletheia.:∧, Aletheia.:∨, Aletheia.:→, extras...))
    )
end
function parseformula(
    expr::AbstractString;
    atom_parser=identity,
    additional_operators=nothing,
    ...
    pool =
        additional_operators === nothing ? _DEFAULT_POOL : _parse_pool(additional_operators)
    return _wrap(
        Aletheia.parse(pool, expr; atom_parser=text -> _payload(atom_parser, text))
    )
end
function parseformula(::Type{<:Aletheia.Formula}, expr::AbstractString; kwargs...)
    return parseformula(expr; kwargs...)
end
function parseformula(expr::AbstractString, additional_operators; kwargs...)
    return parseformula(expr; additional_operators=additional_operators, kwargs...)
end
function parseformula(
    ::Type{<:Aletheia.Formula}, expr::AbstractString, additional_operators; kwargs...
)
    return parseformula(expr; additional_operators=additional_operators, kwargs...)
end
...
check(formula::Aletheia.Formula, args...) = Aletheia.check(_unwrap(formula), args...)
function interpret(formula::Aletheia.Formula, args...)
    return Aletheia.interpret(_unwrap(formula), args...)
end
...
function collateworlds(frame, connective, truth_sets::Tuple)
    ...
    return Aletheia.collateworlds(frame, native, truth_sets)
end
...
struct LeftmostLinearForm{C,SS} <: Aletheia.Formula
    ...
    function LeftmostLinearForm{C,SS}(
        grandchildren::AbstractVector, allow_empty::Bool=false
    ) where {C,SS}
        ...
        return new{C,SS}(convert(Vector{SS}, grandchildren))
    end
end
...
function _element_type(grandchildren::AbstractVector)
    ...
    return reduce(typejoin, map(typeof, grandchildren))
end
function LeftmostLinearForm{C}(grandchildren::AbstractVector, args...) where {C}
    return LeftmostLinearForm{C,_element_type(grandchildren)}(grandchildren, args...)
end
function LeftmostLinearForm(
    connective::_AnyConnective, grandchildren::AbstractVector, args...
)
    return LeftmostLinearForm{typeof(_native_operator(connective))}(grandchildren, args...)
end
...
function LeftmostLinearForm(formula::Aletheia.Formula, connective=nothing, args...)
    formula isa LeftmostLinearForm &&
        return LeftmostLinearForm(tree(formula), connective, args...)
    native = connective === nothing ? nothing : _native_operator(connective)
    if native === nothing
        (formula isa _CompatBranch || formula isa Aletheia.Branch) || _unsupported(
            :LeftmostLinearForm,
            "a leaf cannot be flattened without an explicit connective",
        )
        native = _native_connective(formula)
    end
    return LeftmostLinearForm(native, _flatten(_wrap(formula), typeof(native)), args...)
end
function LeftmostLinearForm{C}(formula::Aletheia.Formula, args...) where {C}
    return LeftmostLinearForm(formula, C(), args...)
end
...
function Base.getindex(form::LeftmostLinearForm{C}, indices::AbstractVector) where {C}
    return LeftmostLinearForm{C}(form.grandchildren[indices])
end
...
function Base.:(==)(left::LeftmostLinearForm{C}, right::LeftmostLinearForm{C}) where {C}
    return left.grandchildren == right.grandchildren
end
...
function Literal(formula::Union{Atom,Aletheia.Atom,Truth}, flag::Bool=true)
    return Literal{typeof(formula)}(flag, formula)
end
function Literal(formula::Union{_CompatBranch,Aletheia.Branch}, flag::Bool=true)
    _native_connective(formula) isa Aletheia.Negation ||
        _unsupported(:Literal, "cannot construct a Literal from $(syntaxstring(formula))")
    return Literal(_wrap(children(formula)[1]), !flag)
end
...
function Base.:(==)(left::Literal, right::Literal)
    return left.ispos == right.ispos && left.atom == right.atom
end
...
function tree(literal::Literal)
    return literal.ispos ? tree(literal.atom) : Branch(Aletheia.:¬, tree(literal.atom))
end
function tree(form::LeftmostLinearForm{C}) where {C}
    c = C()
    subtrees = [tree(child) for child in form.grandchildren]
    length(subtrees) == 1 &&
        return Aletheia.arity(c) == 1 ? Branch(c, subtrees[1]) : subtrees[1]
    return foldr((left, right) -> Branch(c, left, right), subtrees)
end
...
function syntaxstring(form::LeftmostLinearForm{C}; kwargs...) where {C}
    ...
    return join((_bracket(child; kwargs...) for child in form.grandchildren), separator)
end
...
function _bracket(child; kwargs...)
    ...
    return occursin(' ', text) ? "(" * text * ")" : text
end
...
function check(form::Union{LeftmostLinearForm,Literal}, model::Aletheia.Model, args...)
    return check(tree(form), model, args...)
end
function interpret(form::Union{LeftmostLinearForm,Literal}, model::Aletheia.Model, args...)
    return interpret(tree(form), model, args...)
end
...
struct ExplicitAlphabet{V} <: AbstractAlphabet{V}
    ...
    function ExplicitAlphabet(values)
        return ExplicitAlphabet([
...
function atoms(alphabet::UnionAlphabet)
    return reduce(vcat, atoms.(alphabet.subalphabets); init=Aletheia.Formula[])
end
...
function _rng(rng)
    rng isa Integer && _unsupported(
        :randformula, "an integer seed needs Random; pass an AbstractRNG instead"
    )
    return rng
end
...
function randformula(
    ...
    isempty(kwargs) || _unsupported(
        :randformula, "unsupported keyword(s) $(join(string.(keys(kwargs)), ", "))"
    )
    rng = _rng(rng)
    domain = _atom_domain(alphabet)
    isempty(domain) &&
        basecase === nothing &&
        throw(ArgumentError("cannot generate formulas from an empty alphabet"))
    ...
    elseif atompicker === nothing
        ...
        length(weights) == length(domain) || throw(
            ArgumentError(
    ...
    weights === nothing ||
        length(weights) == length(operators) ||
        throw(
            ArgumentError(
    ...
    isempty(nonmodal) &&
        maxmodaldepth < maxheight &&
        throw(
            ArgumentError("no non-modal operator is available below the modal depth limit")
        )
    function generate(height::Integer, modaldepth::Integer, must_honor::Bool)
        if height == 0 ||
           (mode != :full && !must_honor && _rand(rng, Float64) < earlystoppingtreshold)
            return basecase === nothing ? picker(rng, alphabet) : basecase(rng)
        end
        candidates, candidate_weights = if modaldepth > 0
            (natives, weights)
        else
        ...
        subformulas = [
            generate(
                ...
                must_honor && index == honored,
            ) for index in 1:n
        ]
        return Branch(operator, subformulas...)
    end
    return generate(maxheight, maxmodaldepth, mode != :maxheight)
end
function randformula(maxheight::Integer, alphabet, operators::AbstractVector; kwargs...)
    return randformula(nothing, maxheight, alphabet, operators; kwargs...)
end
...
const RCC8Relations = (
    Topo_DC, Topo_EC, Topo_PO, Topo_TPP, Topo_TPPi, Topo_NTPP, Topo_NTPPi
)
const TruthDict = Aletheia.Valuation
function KripkeStructure(frame_value, valuation_value)
    return Aletheia.Model(frame_value, Aletheia.BOOLEAN, valuation_value)
end

# Names used by Sole's modal and collection helpers.
const IARelations = (
    IA_A, IA_L, IA_B, IA_E, IA_D, IA_O, IA_Ai, IA_Li, IA_Bi, IA_Ei, IA_Di, IA_Oi
)
function alphabet(args...)
    return _unsupported(
        :alphabet,
        "Aletheia has no model-wide alphabet object; collect Atom payloads from a Formula",
    )
end
function feature(args...)
    return _unsupported(
        :feature, "feature metadata belongs to SoleData conditions, not Aletheia syntax"
    )
end
function condition(args...)
    return _unsupported(
        :condition, "condition metadata belongs to SoleData conditions, not Aletheia syntax"
    )
end
function threshold(args...)
    return _unsupported(
        :threshold, "threshold metadata belongs to SoleData conditions, not Aletheia syntax"
    )
end
function normalize(args...)
    return _unsupported(
        :normalize, "Aletheia's normal-form conversion is explicit: use cnf or dnf"
    )
end
function sample(args...)
    return _unsupported(
        :sample,
        "random SoleLogics generation is not part of Aletheia's compatibility surface",
    )
end
function worldtype(::Aletheia.Frame)
    return _unsupported(
        :worldtype,
        "Aletheia frames are heterogeneous tuples; use eltype(worlds(frame)) explicitly",
    )
end
...
using ..Truth: Truth
using ..BooleanTruth: BooleanTruth
using ..istop: istop
using ..isbot: isbot
using ..syntaxstring: syntaxstring
import Base: convert
...
function FiniteTruth(index::Integer)
    ...
    return FiniteTruth(UInt8(index))
end
...
function Base.convert(::Type{FiniteTruth}, value::BooleanTruth)
    return FiniteTruth(istop(value) ? UInt8(1) : UInt8(2))
end
...
function Base.convert(::Type{FiniteTruth}, value::Char)
    ...
    return error("Please, provide a character between α and ҭ, ⊤ and ⊥")
end
function Base.convert(::Type{FiniteTruth}, value::AbstractString)
    return if length(value) == 1
        convert(FiniteTruth, only(value))
    else
...
function syntaxstring(value::FiniteTruth; kwargs...)
    return if value.index < UInt8(3)
        Char(UInt16(8867) + value.index)
    else
...
@inline function (operation::_FiniteOperation)(left, right)
    return FiniteTruth(operation.table[Int(_index(left)), Int(_index(right))])
end
function Base.getindex(operation::_FiniteOperation, left, right)
    return FiniteTruth(operation.table[Int(_index(left)), Int(_index(right))])
end
...
function _operation_table(operation, n::Int, name::AbstractString)
    ...
    elseif operation isa AbstractVector || operation isa Tuple
        length(operation) == n * n ||
            throw(ArgumentError("$name table must contain $(n * n) entries"))
...
function Base.show(io::IO, ::MIME"text/plain", algebra::FiniteFLewAlgebra{N}) where {N}
    function table(operation)
        return if N <= 10 || Aletheia._display_limit(io) == typemax(Int)
    ...
    for (label, content) in (
        ("Domain", getdomain(algebra)),
        ("Bot", algebra.bot),
        ("Top", algebra.top),
        ("Join", table(algebra.join)),
        ("Meet", table(algebra.meet)),
        ("T-norm", table(algebra.monoid)),
        ("Implication", table(algebra.implication)),
    )
...
function _wrap_algebra(native::Aletheia.FiniteFLewAlgebra{N}) where {N}
    return FiniteFLewAlgebra{N}(
        ...
        FiniteTruth(native.bot),
        FiniteTruth(native.top),
        native,
    )
end
...
function FiniteFLewAlgebra(join, meet, monoid, bot, top)
    ...
    return FiniteFLewAlgebra{n}(join, meet, monoid, bot, top)
end
...
@inline function precedes(algebra::FiniteFLewAlgebra, left, right)
    l, r = convert(FiniteTruth, left), convert(FiniteTruth, right)
    return l != r && precedeq(algebra, l, r)
end
# Deliberately mirrors SoleLogics' spelling for compatibility.
@inline succeedes(algebra::FiniteFLewAlgebra, left, right) = precedes(algebra, right, left)
...
function precedeq(algebra::Aletheia.FiniteFLewAlgebra, left, right)
    return precedeq(_wrap_algebra(algebra), left, right)
end
function precedes(algebra::Aletheia.FiniteFLewAlgebra, left, right)
    return precedes(_wrap_algebra(algebra), left, right)
end
function succeedeq(algebra::Aletheia.FiniteFLewAlgebra, left, right)
    return succeedeq(_wrap_algebra(algebra), left, right)
end
function succeedes(algebra::Aletheia.FiniteFLewAlgebra, left, right)
    return succeedes(_wrap_algebra(algebra), left, right)
end
function maximalmembers(algebra::Aletheia.FiniteFLewAlgebra, threshold)
    return maximalmembers(_wrap_algebra(algebra), threshold)
end
function minimalmembers(algebra::Aletheia.FiniteFLewAlgebra, threshold)
    return minimalmembers(_wrap_algebra(algebra), threshold)
end

function _unsupported(name::Symbol, detail::AbstractString)
    return throw(
        ArgumentError(
            "SoleLogics.ManyValuedLogics.$name has no faithful Aletheia equivalent: $detail"
        ),
    )
end
function precedeq(args...)
    return _unsupported(:precedeq, "the first argument must be a finite FLew algebra")
end
function succeedeq(args...)
    return _unsupported(:succeedeq, "the first argument must be a finite FLew algebra")
end
function maximalmembers(args...)
    return _unsupported(:maximalmembers, "the first argument must be a finite FLew algebra")
end
function minimalmembers(args...)
    return _unsupported(:minimalmembers, "the first argument must be a finite FLew algebra")
end
...
function (::Type{ContinuousTruth})(args...)
    return _unsupported(
        :ContinuousTruth,
...
export booleanalgebra,
    precedeq, precedes, succeedeq, succeedes, maximalmembers, minimalmembers
...
function _contains_truth(formula)
    ...
    return any(node -> _truth_payload(node) !== nothing, formulas(formula))
end
function _truth_carrier(truth::Truth, algebra::Aletheia.TruthAlgebra)
    ...
    elseif truth isa ManyValuedLogics.FiniteTruth
        if algebra isa Aletheia.FiniteFLewAlgebra
            truth.index <= length(algebra) || throw(
                ArgumentError(
    ...
    return _unsupported(
        :check, "truth leaf $(repr(truth)) cannot be interpreted in $(typeof(algebra))"
    )
end
function _truth_model(model::Aletheia.Model)
    ...
    return Aletheia.Model(Aletheia.frame(model), Aletheia.algebra(model), valuation)
end
function check(formula::Truth, model::Aletheia.Model, world)
    return _truth_carrier(formula, Aletheia.algebra(model))
end
function check(formula::Aletheia.Formula, model::Aletheia.Model, args...)
    native = _unwrap(formula)
    return if _contains_truth(formula)
        Aletheia.check(native, _truth_model(model), args...)
    else
...
function interpret(formula::Aletheia.Formula, model::Aletheia.Model, args...)
    native = _unwrap(formula)
    return if _contains_truth(formula)
        Aletheia.interpret(native, _truth_model(model), args...)
    else
...
export token,
    op, tree, children, value, nchildren, arity, syntaxstring, hasdual, dual, relation
export formulas,
    subformulas, atoms, leaves, connectives, operators, ntokens, natoms, nleaves
...
export AbstractAlphabet,
    ExplicitAlphabet, UnionAlphabet, subalphabets, randatom, randformula
...
function _literal(formula, literaltype::Type)
    literaltype === Literal ||
        _unsupported(:dnf, "literal type $(literaltype) is not supported; use Literal")
    if (formula isa _CompatBranch || formula isa Aletheia.Branch) &&
       _native_connective(formula) isa Aletheia.Negation
        return Literal(false, _wrap(children(formula)[1]))
    end
    return Literal(true, _wrap(formula))
end
function _normal_form(formula::Aletheia.Formula, native, outer, inner, literaltype)
    ...
    return LeftmostLinearForm(
        outer(),
        [
            LeftmostLinearForm(
                inner(),
                [_literal(literal, literaltype) for literal in _flatten(term, inner)],
            ) for term in terms
        ],
    )
end
function dnf(formula::Aletheia.Formula, literaltype::Type=Literal; kwargs...)
    isempty(kwargs) ||
        _unsupported(:dnf, "Aletheia's normal forms take no normalization keyword(s)")
    return _normal_form(
        formula, Aletheia.dnf, Aletheia.Disjunction, Aletheia.Conjunction, literaltype
    )
end
function cnf(formula::Aletheia.Formula, literaltype::Type=Literal; kwargs...)
    isempty(kwargs) ||
        _unsupported(:cnf, "Aletheia's normal forms take no normalization keyword(s)")
    return _normal_form(
        formula, Aletheia.cnf, Aletheia.Conjunction, Aletheia.Disjunction, literaltype
    )
end
...
function Aletheia.carrier(algebra::ManyValuedLogics.FiniteFLewAlgebra)
    return ManyValuedLogics.getdomain(algebra)
end
Aletheia.truth_type(::ManyValuedLogics.FiniteFLewAlgebra) = ManyValuedLogics.FiniteTruth
function Aletheia.domain(algebra::ManyValuedLogics.FiniteFLewAlgebra)
    return ManyValuedLogics.getdomain(algebra)
end

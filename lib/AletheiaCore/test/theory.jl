...
function theory_random_formula(pool, p, q, rng, depth)
    ...
    choice == 2 &&
        return branch(pool, Diamond(:R), theory_random_formula(pool, p, q, rng, depth - 1))
    choice == 3 &&
        return branch(pool, Box(:R), theory_random_formula(pool, p, q, rng, depth - 1))
    connective = choice == 4 ? (∧) : (∨)
    return branch(
        pool,
        connective,
...
@testset "standard translation and first-order core" begin
    ...
    @test string(
        Aletheia.FOAnd(
            ...
            FOOr(Predicate(:q, Variable(:x)), Predicate(:r, Variable(:x))),
        ),
    ) == "p(x) ∧ (q(x) ∨ r(x))"
    interpretation = FirstOrderInterpretation(
        (1, 2), Dict(:p => Set([1, 2]), :R => Set([(1, 2)]))
    )
    ...
    @test evaluate(
        Forall(
            Variable(:y),
            Aletheia.FOImplies(
                Predicate(:R, Variable(:x), Variable(:y)), Predicate(:p, Variable(:y))
            ),
        ),
        interpretation,
        Dict(:x => 1),
    ...
    formula = branch(
        pool, ∧, branch(pool, Diamond(:R), p), branch(pool, Box(:R), branch(pool, →, p, q))
    )
    ...
    random_formulas = (
        ...
        branch(pool, Box(:R), branch(pool, →, p, q)),
    )
    for trial in 1:8
        ...
        @test all(
            evaluate(standard_translation(random_formula), random_fo, Dict(:x => world)) ==
    ...
    for trial in 1:24
        ...
        random_adjacency = Dict(
            world => [target for target in random_worlds if rand(rng, Bool)] for
            world in random_worlds
        )
        random_frame = Frame(random_worlds, Dict(:R => random_adjacency); index=true)
        random_valuation = Dict(
            name => Set(world for world in random_worlds if rand(rng, Bool)) for
            name in ("p", "q")
        )
        ...
        @test all(
            evaluate(translation, interpretation, Dict(:x => world)) ==
...
@testset "bisimulation and contraction" begin
    ...
    bad = Model(
        Frame((:a, :b), Dict(:R => Dict(:a => [], :b => [])); index=true),
        BOOLEAN,
        Dict("p" => Set([:a])),
    )
    ...
    callback_error = try
        Aletheia._valuation_atoms(callback_model)
    catch error
        error
    end
    ...
    @test [check(p, callback_model, world) for world in worlds(frame(callback_model))] == [
        check(p, callback_quotient, contraction_world(callback_quotient, world)) for
        world in worlds(frame(callback_model))
    ]
    relation_error = try
        Aletheia._model_relation_names(Frame((1,), (world, relation) -> ()))
    catch error
        error
    end
    ...
    @test [check(p, nested_model, world) for world in worlds(frame(nested_model))] == [
        check(p, nested_quotient, contraction_world(nested_quotient, world)) for
        world in worlds(frame(nested_model))
    ]
    ...
    unknown_valuation_model = Model(
        Frame((1,); index=true), BOOLEAN, TheoryUnknownValuation()
    )
    ...
    @test [
        interpret(ambiguous_atom, ambiguous_model, world) for
        world in worlds(ambiguous_frame)
    ] == [true, false]
    ...
    @test [check(p, interval_model, world) for world in interval_worlds] == [
        check(
            p, interval_before_quotient, contraction_world(interval_before_quotient, world)
        ) for world in interval_worlds
    ]

    redundant = Frame(
        (1, 2, 3), Dict(:R => Dict(1 => [2, 3], 2 => [2, 3], 3 => [2, 3])); index=true
    )
    ...
    for trial in 1:128
        ...
        gate_edges = Dict(
            w => [target for target in gate_worlds if rand(gate_rng, Bool)] for
            w in gate_worlds
        )
        gate_frame = Frame(gate_worlds, Dict(:R => gate_edges); index=true)
        gate_valuation = Dict(
            name => Set(w for w in gate_worlds if rand(gate_rng, Bool)) for
            name in ("p", "q")
        )
        ...
        gate_quotient = bisimulation_contraction(
            gate_model; atoms=["p", "q"], relations=[:R]
        )
        gate_qmodel = model(gate_quotient)
        for _ in 1:16
            gate_formula = theory_random_formula(
                gate_pool, gp, gq, gate_rng, rand(gate_rng, 1:5)
            )
            for world in gate_worlds
                @test check(gate_formula, gate_model, world) == check(
                    gate_formula, gate_qmodel, contraction_world(gate_quotient, world)
                )
            end
        end
    end
...
@testset "normal forms" begin
    ...
    for values in (
        ...
        valuation = Dict(
            name => (values[i] ? Set([:w]) : Set{Symbol}()) for
            (i, name) in enumerate(("p", "q", "r"))
        )
    ...
    for trial in 1:24
        random_formula = theory_random_formula(pool, p, q, rng, 3)
        random_valuation = Dict(
            name => (rand(rng, Bool) ? Set([:w]) : Set{Symbol}()) for name in ("p", "q")
        )
        random_model = Model(frame0, BOOLEAN, random_valuation)
        random_cnf, random_dnf = to_cnf(random_formula), to_dnf(random_formula)
        @test check(random_formula, random_model, :w) ==
              check(random_cnf, random_model, :w) ==
...
@testset "proof-search interface" begin
    ...
    @test any(
        check(sat_formula, sat_result.countermodel, world) for
        world in worlds(frame(sat_result.countermodel))
    )
    ...
    needs_two_worlds = branch(
        pool, Diamond(:R), branch(pool, ∧, p, branch(pool, ¬, branch(pool, Diamond(:R), p)))
    )
    ...
    @test check(p, godel_sat.countermodel, first(worlds(frame(godel_sat.countermodel)))) ==
          top(G3)
...
@testset "theory edge cases" begin
    ...
    @test string(
        FONot(Aletheia.FOAnd(Predicate(:p, Variable(:x)), Predicate(:q, Variable(:x))))
    ) == "¬(p(x) ∧ q(x))"
    @test string(
        Aletheia.FOImplies(Predicate(:p, Variable(:x)), Predicate(:q, Variable(:x)))
    ) == "p(x) → q(x)"
    @test string(FOExists(Variable(:x), Predicate(:p, Variable(:x)))) == "∃x. p(x)"
    @test string(FOForall(Variable(:x), Predicate(:p, Variable(:x)))) == "∀x. p(x)"
    @test string(TheoryUnknownTerm()) == "Main.TheoryUnknownTerm()" ||
          string(TheoryUnknownTerm()) isa String
    ...
    @test_throws ArgumentError evaluate(
        TheoryUnknownFO(), FirstOrderInterpretation((1,), Dict())
    )
    fi = FirstOrderInterpretation(
        (1, 2),
        Dict(:d => Dict((1,) => true), :e => Dict(1 => true), :R => Dict((1, 2) => true)),
    )
    ...
    @test_throws KeyError evaluate(
        Predicate(:empty, Variable(:x)),
        FirstOrderInterpretation((1,), Dict(:empty => Dict())),
        Dict(:x => 1),
    )
    @test evaluate(
        FODisjunction(Predicate(:d, Variable(:x)), Predicate(:e, Variable(:x))),
        fi,
        Dict(:x => 1),
    )
    @test evaluate(
        Aletheia.FOAnd(Predicate(:d, Variable(:x)), Predicate(:e, Variable(:x))), fi, (x=1,)
    )
    ...
    @test_throws ArgumentError first_order_interpretation(
        Model(frame, GodelAlgebra(), Dict("p" => Dict(1 => 0.5)))
    )
    callable_model = Model(Frame((1,), (w, r) -> ()), BOOLEAN, (a, w) -> true)
    @test_throws ArgumentError first_order_interpretation(callable_model)
    @test_throws ArgumentError first_order_interpretation(
        Model(Frame((1,), (w, r) -> ()), BOOLEAN, Dict("p" => Set([1])))
    )
    ...
    atom_quotient = bisimulation_contraction(
        atom_model; atoms=[atom_handle], relations=[:R]
    )
    ...
    pair_model = Model(
        pair_frame,
        BOOLEAN,
        Dict((pair_atom, 1) => true, (2, pair_atom) => false, 1 => Dict("q" => true)),
    )
        ...
        first(
            classes(
                bisimulation_contraction(
                    Model(frame, BOOLEAN, Dict("p" => Set([1])));
                    atoms=["p"],
                    relations=[:R],
                ),
            ),

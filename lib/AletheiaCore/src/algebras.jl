...
struct FiniteFLewAlgebra{N} <: TruthAlgebra{FiniteTruth}
    ...
    function FiniteFLewAlgebra{N}(
        ...
        bot::FiniteTruth,
        top::FiniteTruth,
        ::_ValidatedFiniteFLew,
    ) where {N}
        return new{N}(join, meet, fusion, implication, bot, top)
    end
end
...
@inline function _finite_leq(
    meet_table::AbstractMatrix{FiniteTruth}, x::FiniteTruth, y::FiniteTruth
)
    return meet_table[Int(x), Int(y)] == x
end

function _invalid_flew(axiom::AbstractString, witness, detail="")
    suffix = isempty(detail) ? "" : ": $detail"
    return throw(
        ArgumentError("invalid FLew-algebra: $axiom; witness $(repr(witness))$suffix")
    )
end
...
function _validate_lattice(join_table, meet_table, bot, top, n)
    for x in 1:n, y in 1:n
        ...
        jxy == jyx ||
            _invalid_flew("lattice commutativity (join)", (x, y), "join[x,y] != join[y,x]")
        mxy, myx = meet_table[x, y], meet_table[y, x]
        mxy == myx ||
            _invalid_flew("lattice commutativity (meet)", (x, y), "meet[x,y] != meet[y,x]")
...
function _validate_fusion(fusion, meet_table, top, n)
    for x in 1:n, y in 1:n
        fusion[x, y] == fusion[y, x] || _invalid_flew("fusion commutativity", (x, y))
    ...
    for x in 1:n, y in 1:n, z in 1:n
        if _finite_leq(meet_table, FiniteTruth(x), FiniteTruth(y))
            fusion[x, z] == fusion[y, z] ||
                _finite_leq(meet_table, fusion[x, z], fusion[y, z]) ||
                _invalid_flew("fusion monotonicity (left)", (x, y, z))
            fusion[z, x] == fusion[z, y] ||
                _finite_leq(meet_table, fusion[z, x], fusion[z, y]) ||
...
function _derive_implication(join_table, meet_table, fusion, bot, top, n)
    ...
    for x in 1:n, z in 1:n
        ...
        for y in 1:n
            _finite_leq(meet_table, fusion[x, y], FiniteTruth(z)) &&
                push!(candidates, FiniteTruth(y))
        end
        greatest = FiniteTruth[]
        for candidate in candidates
            all(other -> _finite_leq(meet_table, other, candidate), candidates) &&
                push!(greatest, candidate)
        end
        length(greatest) == 1 || _invalid_flew(
            "residuum existence",
            (x, z),
    ...
    for x in 1:n, y in 1:n, z in 1:n
        ...
        left == right ||
            _invalid_flew("residuation", (x, y, z), "x ⊗ y ≤ z iff x ≤ y → z fails")
    end
    return implication
end
...
function _make_flew(join_table, meet_table, fusion_table, bot, top, n::Int)
    ...
    return FiniteFLewAlgebra{n}(
        _immutable_copy(join),
        _immutable_copy(lattice_meet),
        _immutable_copy(fusion),
        _immutable_copy(implication),
        b,
        t,
        _validated_flew,
    )
...
function FiniteFLewAlgebra(join_table, meet_table, fusion_table, bot, top)
    n = if meet_table isa AbstractMatrix
        size(meet_table, 1) == size(meet_table, 2) ||
            throw(ArgumentError("meet table must be square"))
        size(meet_table, 1)
    elseif meet_table isa AbstractVector || meet_table isa Tuple
        r = isqrt(length(meet_table))
        r * r == length(meet_table) ||
            throw(ArgumentError("meet table must contain a square number of entries"))
        r
    else
        throw(ArgumentError("meet table must be an N×N integer matrix"))
    end
    n >= 1 ||
        throw(ArgumentError("a finite FLew-algebra must have at least one truth value"))
    n <= typemax(FiniteTruth) ||
        throw(ArgumentError("FiniteFLewAlgebra supports at most 255 truth values"))
    return FiniteFLewAlgebra{n}(join_table, meet_table, fusion_table, bot, top)
end
...
function FiniteFLewAlgebra{N}(join_table, meet_table, fusion_table, bot, top) where {N}
    ...
    return _make_flew(join_table, meet_table, fusion_table, bot, top, Int(N))
end
...
@inline function _checked_finite_index(algebra::FiniteFLewAlgebra{N}, value) where {N}
    ...
    1 <= value <= N ||
        throw(ArgumentError("value $(repr(value)) is outside the FiniteTruth carrier 1:$N"))
    return FiniteTruth(value)
end
@inline function _validate_atom_value(algebra::FiniteFLewAlgebra, value)
    return _checked_finite_index(algebra, value)
end
...
function isfinitechain(algebra::FiniteFLewAlgebra)
    ...
    return all(
        x -> all(y -> precedeq(algebra, x, y) || precedeq(algebra, y, x), values), values
    )
end
...
function maximalmembers(algebra::FiniteFLewAlgebra, subset)
    values = if subset isa Integer
        [x for x in domain(algebra) if !succeedeq(algebra, x, subset)]
    else
        _finite_subset(algebra, subset)
    end
    return [x for x in values if !any(y -> y != x && precedes(algebra, x, y), values)]
end
...
function minimalmembers(algebra::FiniteFLewAlgebra, subset)
    values = if subset isa Integer
        [x for x in domain(algebra) if !precedeq(algebra, x, subset)]
    else
        _finite_subset(algebra, subset)
    end
    return [x for x in values if !any(y -> y != x && precedes(algebra, y, x), values)]
end
...
`UInt8` index.
"""
function truthlabel(index::Integer)
    return string(Int(index) < 3 ? Char(8867 + Int(index)) : Char(942 + Int(index)))
end
...
function _display_truth(algebra::FiniteFLewAlgebra{N}, value) where {N}
    ...
    return if value isa Integer && !(value isa Bool) && 1 <= value <= N
        truthlabel(algebra, value)
    else
        string(value)
    end
...
function _algebra_order(algebra::FiniteFLewAlgebra{N}) where {N}
    ...
    ischain = all(
        precedeq(algebra, x, y) || precedeq(algebra, y, x) for x in values, y in values
    )
    ischain && return (sort(values; lt=(x, y) -> precedes(algebra, x, y)), true)
    rest = [v for v in values if v != algebra.bot && v != algebra.top]
    return (unique(vcat(algebra.bot, rest, algebra.top)), false)
end

function _truth_color(algebra::FiniteFLewAlgebra, value)
    return if value == algebra.top
        _DISPLAY_TOP
    elseif value == algebra.bot
        _DISPLAY_BOT
    else
        :normal
    end
...
function Base.show(io::IO, algebra::FiniteFLewAlgebra{N}) where {N}
    return print(
        io,
        "FiniteFLewAlgebra{$N}(bottom=",
        truthlabel(algebra, algebra.bot),
        ", top=",
        truthlabel(algebra, algebra.top),
        ")",
    )
end

function Base.show(io::IO, ::MIME"text/plain", ::BooleanAlgebra)
    return _display_header(io, "BooleanAlgebra", "carrier Bool: {false, true}")
end
...
function _model_algebra_summary(::LukasiewiczAlgebra{0})
    return "LukasiewiczAlgebra (unit interval [0.0, 1.0])"
end
_model_algebra_summary(::GodelAlgebra{N}) where {N} = "GodelAlgebra ($N-level chain)"
function _model_algebra_summary(::LukasiewiczAlgebra{N}) where {N}
    return "LukasiewiczAlgebra ($N-level chain)"
end
...
function Base.show(io::IO, ::MIME"text/plain", alg::GodelAlgebra{N}) where {N}
    ...
    return _display_header(io, "GodelAlgebra{$N}", "chain of $N levels: $lvls")
end
...
function Base.show(io::IO, ::MIME"text/plain", alg::LukasiewiczAlgebra{N}) where {N}
    ...
    return _display_header(io, "LukasiewiczAlgebra{$N}", "chain of $N levels: $lvls")
end

"""Render one operation table as element-labelled lines, in display order."""
function _render_table_rows(
    op_symbol::String, matrix::AbstractMatrix{FiniteTruth}, order, labels
)
    width = maximum(length, labels)
    cell(text) = lpad(text, width)
    lines = String[" " * cell(op_symbol) * " │ " * join(
        (cell(labels[Int(y)]) for y in order), " "
    )]
...
function Base.show(io::IO, ::MIME"text/plain", algebra::FiniteFLewAlgebra{N}) where {N}
    ...
    N <= 10 || return nothing
    tables = (
        _render_table_rows("∧", algebra.meet, order, labels),
    ...
    widths = (
        max(maximum(length, tables[1]), length(titles[1])) + 4,
...
function _chain_index(level::Rational, n::Int)
    ...
    return FiniteTruth(Int(level * (n - 1)) + 2)
end
...
function _chain_flew(n::Int, kind::Symbol)
    ...
    for i in 1:n, j in 1:n
        ...
        monoid_level = if kind === :godel
            min(levels_[i], levels_[j])
        else
            max(0//1, levels_[i] + levels_[j] - 1)
        end

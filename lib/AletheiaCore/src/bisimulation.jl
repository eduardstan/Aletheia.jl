...
function _model_relation_names(frame::Frame)
    ...
    stored isa Function && throw(
        ArgumentError("callable accessibility requires an explicit relations keyword")
    )
    stored isa _RelationProvider && throw(
        ArgumentError(
...
function _label_compatible(m1, w1, m2, w2, atoms)
    return all(isequal(_atom_truth(m1, a, w1), _atom_truth(m2, a, w2)) for a in atoms)
end
function _targets_tuple(model, world, name)
    return Tuple(accessible(frame(model), world, name))
end
...
function bisimilar(m1::Model, w1, m2::Model, w2; atoms=nothing, relations=nothing)
    ...
    _check_world(frame(m2), w2)
    names = if atoms === nothing
        unique(vcat(_valuation_atoms(m1), _valuation_atoms(m2)))
    else
        collect(atoms)
    end
    rels = if relations === nothing
        unique(vcat(_model_relation_names(frame(m1)), _model_relation_names(frame(m2))))
    else
        collect(relations)
    end
    ...
    while changed
        ...
        for pair in collect(pairs)
            ...
            survives =
                all(
                    any(
                        (target, candidate) in pairs for
                        candidate in _targets_tuple(m2, right, rel)
                    ) for rel in rels for target in _targets_tuple(m1, left, rel)
                ) && all(
                    any(
                        (candidate, target) in pairs for
                        candidate in _targets_tuple(m1, left, rel)
...
function Base.show(io::IO, class::BisimulationClass)
...
function Base.show(io::IO, contraction::BisimulationContraction)
    return print(
        ...
        length(contraction.world_map),
        " → ",
        length(contraction.classes),
        " worlds)",
    )
end
...
function accessible(contraction::BisimulationContraction, world, relation_name)
    return accessible(frame(contraction), world, relation_name)
end
function check(formula::Formula, contraction::BisimulationContraction, world)
    return check(formula, contraction.model, contraction_world(contraction, world))
end
function extension(formula::Formula, contraction::BisimulationContraction)
    return extension(formula, contraction.model)
end
...
function _classes_for_model(model::Model, atoms, relations)
    ...
    while true
        ...
        for world in worlds_list
            ...
            successors = Tuple(
                Tuple(
                    sort!(
                        unique(
                            block_index[target] for
                            target in _targets_tuple(model, world, relation_name)
                        ),
                    ),
        ...
        length(next_blocks) == length(blocks) &&
            all(
                any(next_block == block for next_block in next_blocks) for block in blocks
            ) &&
            break
        blocks = next_blocks
    end
    ...
    for block in blocks
        ...
        try
            ...
            sort!(
                members;
                by=world -> findfirst(candidate -> isequal(candidate, world), worlds_list),
            )
        end
...
function bisimulation_contraction(model::Model; atoms=nothing, relations=nothing)
    ...
    return BisimulationContraction(qmodel, tuple(quotient_classes...), mapping)
end

@testset "scalar data preparation and evaluation" begin
    ...
    function AletheiaData.feature_value(x::VersionedScalarSource, i, w, ::Val{:twice})
        return 2x.values[(i, w)]
    end
    ...
    @test batch_apply([modal], prepared; cache=cache) ==
          [[BitVector([1, 0, 0]), BitVector([1, 1, 0])]]
    ...
    @test batch_apply(
        [atom(FormulaPool(Signature((¬,))), ThresholdCondition(Val(:x), >, 0.5))], lazy
    )[1][1] == BitVector([0, 1, 0])
    @test_throws ArgumentError batch_apply(
        [modal], prepared; cache=EvaluationCache(instance_model(family, 1))
    )
end
...
@testset "scalar protocol edge cases" begin
    ...
    @test DenseFeatureStore(
        reshape([1, 2], 2, 1, 1), [:a, :b], [1], [Val(:x)], 5
    ).version == 5
...
@testset "scalar protocol dispatch coverage" begin
    ...
    dense = DenseFeatureStore(
        reshape([1, 2, 3, 4], 2, 2, 1), [:a, :b], [Val(:x)]; instances=[10, 20], version=7
    )
    ...
    @test prepare_scalar(props2; features=[Val(:x)], frames=fr2).store.values[1, 1, 1] ==
          1.0
    @test prepare_scalar(props2; features=[Val(:x)], frames=[fr2]).store.values[2, 1, 1] ==
          2.0
    count_source = CountScalarSource(2)
    count_prep2 = prepare_scalar(
        count_source; features=[Val(:x)], worlds=[1, 2], instances=nothing
    )
    @test count_prep2.store.values[:, 2, 1] == Any[3, 4]
    @test prepare_scalar(
        props2; features=[Val(:x)], precompute_aggregates=true
    ).one_step_memos.global_values !== nothing
    @test prepare_scalar(
        props2; features=[Val(:x)], precompute_aggregates=:all
    ).one_step_memos.global_values !== nothing
    @test prepare_scalar(
        props2; features=[Val(:x)], precompute_aggregates=[Val(:x) => maximum]
    ).one_step_memos.global_values !== nothing
    @test_throws ArgumentError prepare_scalar(
        props2; features=[Val(:x)], precompute_aggregates=[:bad]
    )
    @test_throws ArgumentError prepare_scalar(
        props2; features=[Val(:x)], precompute_aggregates=[(:x, maximum, :extra)]
    )
    ...
    interval_prep = prepare_scalar(
        [[2.0, 5.0]]; features=[v -> minimum(v)], frames=[interval_frame], instances=[1]
    )
    ...
    rectangle_prep = prepare_scalar(
        [reshape([1.0, 2.0, 3.0, 4.0], 2, 2);];
        features=[v -> sum(v)],
        frames=[rectangle_frame],
        instances=[1],
    )
    @test feature_value(
        rectangle_prep, 1, rectangle_world, first(rectangle_prep.store.features)
    ) == 10.0
    point_world = Point(2)
    point_frame = Frame((point_world,), Dict(); index=true)
    point_prep = prepare_scalar(
        [[3.0, 7.0]]; features=[identity], frames=[point_frame], instances=[1]
    )

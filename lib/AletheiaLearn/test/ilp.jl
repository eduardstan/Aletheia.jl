@testset "ILP clauses and substitutions" begin
    ...
    @test evaluate(
        Predicate(:p, FunctionTerm(:f, Variable(:x))),
        FirstOrderInterpretation(
            (1, 2),
            Dict(:p => Set([2]), (:f, 1) => Dict((1,) => 2));
            functions=Dict(:f => Dict((1,) => 2)),
        ),
        Dict(:x => 1),
    )
    @test evaluate(
        Predicate(:p, FunctionTerm(:f, Variable(:x))),
        FirstOrderInterpretation((1, 2), Dict(:p => Set([2])); functions=(name, x...) -> 2),
        Dict(:x => 1),
    )
    @test evaluate(
        Predicate(:p, FunctionTerm(:f, Variable(:x))),
        FirstOrderInterpretation(
            (1, 2), Dict(:p => Set([2])); functions=Dict(:f => (x -> 2))
        ),
        Dict(:x => 1),
    )
    ...
    @test length(horn) == 3
    @test HornClause(nothing, Predicate(:body, X)) ==
          HornClause(Clause(Literal(Predicate(:body, X), false)))
    @test HornClause((Predicate(:head, X), Literal(Predicate(:body, X), false))) isa
          HornClause
    ...
    @test HornClause([Predicate(:head, X), Literal(Predicate(:body, X), false)]) ==
          HornClause(Clause(Predicate(:head, X), Literal(Predicate(:body, X), false)))
    ...
    @test substitute(c, θ) == Clause(
        Predicate(:p, a, FunctionTerm(:g, b)),
    ...
    general = HornClause(
        Predicate(:father, X, Y), Predicate(:parent, X, Y), Predicate(:male, X)
    )
    specific = Clause(
        Predicate(:father, a, b),
        Literal(Predicate(:parent, a, b), false),
        Literal(Predicate(:parent, a, Constant(:ann)), false),
        Literal(Predicate(:male, a), false),
    ...
    q1 = Clause(
        ...
        Literal(Predicate(:mother, X, Y), false),
    ...
    @test subsumes(
        Clause(Predicate(:p, X), Predicate(:q, X)),
    ...
    recursive_d = HornClause(
        Predicate(:p, FunctionTerm(:f, FunctionTerm(:f, Y))), Predicate(:p, Y)
    )
...
@testset "ILP lazy refinements and learning settings" begin
    ...
    @test_throws ArgumentError collect(
        downward_refinements(base; substitutions=[Dict(:X => :bad)])
    )
    @test !isempty(
        collect(
            downward_refinements(
                base; literals=[Predicate(:q, X)], substitutions=Dict(:X => Constant(:a))
            ),
        ),
    )
    templated = collect(
        downward_refinements(
            Clause(Predicate(:p, Constant(:a)));
            literals=[Predicate(:q, X)],
            substitutions=Substitution(:X => Constant(:b)),
        ),
    )
    ...
    horn_negative = collect(
        downward_refinements(horn; literals=[Literal(Predicate(:newbody, X), false)])
    )
    @test !isempty(horn_negative) &&
          all(candidate isa HornClause && ishorn(candidate) for candidate in horn_negative)
    @test all(
        subsumes(horn, candidate) && !subsumes(candidate, horn) for
        candidate in horn_negative
    )
    @test all(
        candidate isa HornClause for candidate in collect(
            downward_refinements(horn; substitutions=[Substitution(:X => Constant(:a))])
        )
    )
    upward_horn = HornClause(
        Predicate(:head, FunctionTerm(:f, X)), Literal(Predicate(:body, X), false)
    )
    upward_horn_candidates = collect(upward_refinements(upward_horn))
    @test !isempty(upward_horn_candidates) &&
          all(candidate isa HornClause for candidate in upward_horn_candidates)
    @test_throws ArgumentError collect(
        downward_refinements(horn; literals=[Predicate(:newhead, X)])
    )
    substitution = Substitution(:X => Constant(:a))
    @test isempty(
        collect(downward_refinements(base; substitutions=[substitution], max_literals=0))
    )
    @test_throws ArgumentError collect(
        downward_refinements(base; substitutions=[substitution], max_literals=-1)
    )
    bounded = collect(
        downward_refinements(base; substitutions=[substitution], max_literals=1)
    )
    ...
    multi = collect(
        downward_refinements(
            base;
            literals=(Predicate(:q, Constant(i)) for i in 1:2),
    ...
    infinite_literals = (
        Literal(Predicate(:q, Constant(i))) for i in Iterators.countfrom(1)
    )
    ...
    @test all(
        subsumes(base, candidate) && !subsumes(candidate, base) for candidate in first_three
    )
    @test length(unique(first_three)) == 3

    up = collect(
        upward_refinements(Clause(Predicate(:p, FunctionTerm(:f, X)), Predicate(:q, X)))
    )
    @test !isempty(up)
    @test all(
        subsumes(candidate, Clause(Predicate(:p, FunctionTerm(:f, X)), Predicate(:q, X))) &&
        !subsumes(Clause(Predicate(:p, FunctionTerm(:f, X)), Predicate(:q, X)), candidate)
        for candidate in up
    )
    @test isempty(collect(upward_refinements(Clause())))
    collision = Clause(
        Predicate(:p, FunctionTerm(:f, Variable(:_G1))), Equality(Constant(:a), X)
    )
...
@testset "ILP hypothesis scoring" begin
    function labelled_model(value; algebra=BOOLEAN, world=:w)
        model = Model(
            Frame((world,), Dict()), Dict("p" => Dict(world => value)); algebra=algebra
        )
        return model
    end
    ...
    result = score(
        hypothesis,
        [positive_covered, negative_covered, negative_uncovered, positive_uncovered],
    )
    ...
    @test (
        no_positives.true_positives,
        no_positives.false_positives,
        no_positives.true_negatives,
        no_positives.false_negatives,
    ) == (0, 1, 1, 0)
    @test no_positives.accuracy == 0.5

    no_negatives = score(hypothesis, [positive_covered, positive_uncovered])
    @test (
        no_negatives.true_positives,
        no_negatives.false_positives,
        no_negatives.true_negatives,
        no_negatives.false_negatives,
    ) == (1, 0, 0, 1)
    ...
    @test (
        covered_by_none.true_positives,
        covered_by_none.false_positives,
        covered_by_none.true_negatives,
        covered_by_none.false_negatives,
    ) == (0, 0, 1, 1)
    ...
    partial_result = score(
        hypothesis,
        [
            InterpretationExample(partial; positive=true),
            InterpretationExample(partial; positive=false),
        ],
    )
    @test (
        partial_result.true_positives,
        partial_result.false_positives,
        partial_result.true_negatives,
        partial_result.false_negatives,
    ) == (0, 0, 1, 1)

    # AnyWorld is existential and does not depend on a first-world/index=1 convention.
    arbitrary_worlds = Model(
        Frame((:dead, :true), Dict()), Dict("p" => Set([:true])); algebra=BOOLEAN
    )
    arbitrary_world_result = score(hypothesis, [InterpretationExample(arbitrary_worlds)])
    @test (
        arbitrary_world_result.true_positives,
        arbitrary_world_result.false_positives,
        arbitrary_world_result.true_negatives,
        arbitrary_world_result.false_negatives,
    ) == (1, 0, 0, 0)
    ...
    many_valued = score(
        hypothesis,
        [
            InterpretationExample(many_valued_top; positive=true),
            InterpretationExample(many_valued_middle; positive=false),
        ],
    )
    @test (
        many_valued.true_positives,
        many_valued.false_positives,
        many_valued.true_negatives,
        many_valued.false_negatives,
    ) == (1, 0, 1, 0)

...
function Measurement(
    time, gctime, allocs, memory, minimum, maximum, max_gctime, samples; note=""
)
    return Measurement(
        time, gctime, allocs, memory, minimum, maximum, max_gctime, samples, note
    )
...
function setup_environment(environment, aletheia_root, consumer_package)
    ...
    return Pkg.instantiate(; io=devnull)
end
...
function run_worker(environment, mode, arguments; timeout=300, allow_timeout=false)
    ...
    path, io = mktemp(ROOT)
    close(io)
    error_path, error_io = mktemp(ROOT)
    close(error_io)
    ...
    process.exitcode == 0 ||
        error("worker failed (exit code $(process.exitcode)): $(error_output)")
    return output, ""
end
...
function compare_gate(baseline, routed, shapes)
    ...
    baseline_lines == routed_lines || begin
        mismatch = findfirst(
            i -> baseline_lines[i] != routed_lines[i],
            1:min(length(baseline_lines), length(routed_lines)),
        )
        mismatch === nothing &&
            (mismatch = min(length(baseline_lines), length(routed_lines)) + 1)
        baseline_row =
            mismatch <= length(baseline_lines) ? baseline_lines[mismatch] : "missing"
        routed_row =
            mismatch <= length(routed_lines) ? routed_lines[mismatch] : "missing"
        error(
            "mask disagreement at gate row $mismatch: baseline=$(repr(baseline_row)) routed=$(repr(routed_row))",
        )
    end
    cases = sum(length(split(split(line, '\t')[3], ',')) for line in baseline_lines)
    return (rows=length(baseline_lines), rule_instance_cases=cases, shapes=length(shapes))
end

function parse_measurement(fields, start; note="")
    values = split(fields[start], ',')
    values[1] == "NA" && return Measurement(
        missing, missing, missing, missing, missing, missing, missing, 0; note=note
    )
    length(values) == 8 || error(
        "measurement must contain time, GC, paired allocations/bytes, range, and sample count",
    )
    return Measurement(
        parse(Float64, values[1]),
        parse(Float64, values[2]),
        parse(Int, values[3]),
        parse(Int, values[4]),
        parse(Float64, values[5]),
        parse(Float64, values[6]),
        parse(Float64, values[7]),
        parse(Int, values[8]);
        note=note,
    )
end
...
function parse_timing(output, ncases, side; note="")
    ...
    missing_row = (
        first_use=missing_measurement,
        steady=missing_measurement,
        churn=missing_measurement,
        adapter=missing_measurement,
        conversion=missing_measurement,
        extension=missing_measurement,
    )
    return [get(rows, i, missing_row) for i in 1:ncases]
end

function fmt_measurement(m)
    m.time === missing && return isempty(m.note) ? "timeout/unavailable" : m.note
    @sprintf(
        "%.3f ms; GC %.3f ms; %d allocs / %d bytes; sample %.3f–%.3f ms; max GC %.3f ms; n=%d",
        m.time / 1e6,
        m.gctime / 1e6,
        m.allocs,
        m.memory,
...
function selected_timing_cases()
    ...
    sort(order) == collect(1:length(TIMING_CASES)) || error(
        "DATASET_CONSUMER_CASE_ORDER must be a permutation of 1:$(length(TIMING_CASES))"
    )
    return TIMING_CASES[order]
end
...
try
    ...
    baseline_gate, baseline_gate_note = run_worker(
        baseline_environment, "gate", encoded_gate
    )
    routed_gate, routed_gate_note = run_worker(routed_environment, "gate", encoded_gate)
    isempty(baseline_gate_note) && isempty(routed_gate_note) ||
        error("agreement gate timed out")
    gate = compare_gate(baseline_gate, routed_gate, GATE_SHAPES)
    println(
        "mask gate: PASS; seed=0xDADA_2024; shapes=$(gate.shapes); " *
        "rule-instance masks=$(gate.rule_instance_cases)",
    )

    timing_cases = selected_timing_cases()
    encoded_timing = join(timing_cases, ";")
    baseline_output, baseline_note = run_worker(
        baseline_environment, "timing", "baseline $encoded_timing"; allow_timeout=true
    )
    routed_output, routed_note = run_worker(
        routed_environment, "timing", "aletheia $encoded_timing"; allow_timeout=true
    )
    baseline_timing = parse_timing(
        baseline_output, length(timing_cases), "baseline"; note=baseline_note
    )
    routed_timing = parse_timing(
        routed_output, length(timing_cases), "aletheia"; note=routed_note
    )

    result_path = get(
        ENV,
        "DATASET_CONSUMER_RESULT",
    ...
    open(result_path, "w") do io
        ...
        println(
            io,
            "gate=PASS shapes=$(gate.shapes) rule-instance-masks=$(gate.rule_instance_cases)",
        )
        println(
            io,
            "case | baseline first use | routed first use | baseline steady | routed steady | baseline churn | routed churn | adapter | formula conversion | extension/mask",
        )
        for (index, case) in enumerate(timing_cases)
            b, a = baseline_timing[index], routed_timing[index]
            println(
                io,
                "$(case) | $(fmt_measurement(b.first_use)) | $(fmt_measurement(a.first_use)) | " *

...
function run_gate()
    gate_shapes = ((1, 3, 1, 0.0), (3, 4, 3, 0.5), (4, 5, 4, 1.0), (7, 6, 5, 0.35))
    ...
    for (ninstances, npoints, depth, modal_probability) in gate_shapes
        ...
        for _ in 1:20
            formula_a, formula_s = make_pair(
                depth, modal_probability, rand(rng, 1:typemax(Int)); supported=true
            )
...
result_path = get(
    ENV,
    "DATASET_PROTOCOL_SUPPORTED_RESULT",
...
println(
    "SupportedLogiset agreement gate: PASS; seed=$(GATE_SEED); formulas=$(gate.formula_count); formula-instance-world cases=$(gate.formula_world_cases)",
)
...
sides = (
    "supported-cold",
    "supported-warm",
    "supported-aletheia-batch",
    "supported-aletheia-scalar",
)
...
open(result_path, "a") do io
    ...
    println(
        io,
        "case(points) | Supported cold first-check | Supported warm repeated-check | Aletheia batch | Aletheia scalar",
    )
    for (index, argument) in enumerate(cases)
        println(
            io,
            "$(argument) | $(fmt(measurements["supported-cold"][index])) | " *
...
println(
    "SupportedLogiset timing results written to $(result_path); cases=$(length(cases)); seed=$(GATE_SEED)",
)

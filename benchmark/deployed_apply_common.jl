...
function build_apply_fixture(;
    ninstances=APPLY_NINSTANCES,
    npoints=APPLY_NPOINTS,
    ...
    tree = ModalDecisionTrees.build_tree(
        ...
        labels;
        min_samples_leaf=2,
        max_depth=APPLY_DEPTH,
        rng=MersenneTwister(train_seed),
    ...
    decision_list = SoleModels.DecisionList(
        all_rules[1:(end - 1)], SoleModels.consequent(all_rules[end])
    )
    scalar_state = DecisionListBatchAdapter.prepare(
        decision_list, modalities; vectorized=false
    )
    vector_state = DecisionListBatchAdapter.prepare(
        decision_list, modalities; vectorized=true
    )
    bridge_scalar = if scalar_layer
        build_bridge_scalar_layer_state(dataset, vector_state; vectorized=false)
    else
        nothing
    end
    bridge_vectorized = if scalar_layer
        build_bridge_scalar_layer_state(dataset, vector_state; vectorized=true)
    else
        nothing
    end
    dense_scalar = if scalar_layer
        build_dense_scalar_layer_state(dataset, vector_state; vectorized=false)
    else
        nothing
    end
    dense_vectorized = if scalar_layer
        build_dense_scalar_layer_state(dataset, vector_state; vectorized=true)
    else
        nothing
    end
    return ApplyFixture(
...
function _relation_frames(dataset, formulas)
    signature_connectives = collect(Aletheia.connectives(Aletheia.signature(formulas[1])))
    relations = unique([
        Aletheia.relation(connective) for connective in signature_connectives if
    ...
    frames = [
        begin
            ...
            adjacency = Dict(
                relation => Dict(
                    world => Tuple(
                        SoleData.accessibles(
                            source_frame, world, getfield(relation, :source)
                        ),
                    ) for world in SoleData.allworlds(source_frame)
                ) for relation in relations
            )
            Aletheia.Frame(
                collect(SoleData.allworlds(source_frame)), adjacency; index=true
            )
...
function build_bridge_scalar_layer_state(dataset, callback_state; vectorized)
    ...
    prepared_source = Aletheia.prepare_scalar(
        dataset;
        relation=SoleLogics.globalrel,
    ...
    prepared = Aletheia.prepare_scalar(
        prepared_source;
        frames=frames,
        relations=relations,
        instances=collect(1:SoleData.ninstances(dataset)),
    )
    return ScalarLayerState(
        prepared,
        callback_state.formulas,
...
function build_dense_scalar_layer_state(dataset, callback_state; vectorized)
    ...
    values = Array{Float64}(
        undef, length(worlds), SoleData.ninstances(dataset), length(features)
    )
    for (iw, world) in enumerate(worlds),
    ...
    store = Aletheia.DenseFeatureStore(
        values, worlds, features; instances=collect(1:SoleData.ninstances(dataset))
    )
    prepared = Aletheia.prepare_scalar(
        store;
        frames=frames,
    ...
    return ScalarLayerState(
        prepared, formulas, Aletheia.scalar_family(prepared; vectorized=vectorized)
    )
end
    ...
    for (formula, _) in zip(state.formulas, fixture.rules)
        ...
        for instance in 1:SoleData.ninstances(fixture.dataset)
            ...
            push!(
                per_instance,
                BitVector(
                    Aletheia.check(formula, state.family, instance, world) for
                    world in worlds
                ),
            )
        end
...
function native_antecedent_masks(fixture)
    return [
        BitVector(SoleModels.checkantecedent(rule, fixture.modalities)) for
        rule in fixture.rules[1:(end - 1)]
    ]
end
...
function run_parity_gate(fixture=build_apply_fixture())
    ...
    scalar_masks_from_extensions == native_masks ||
        error("scalar formula extension mismatch")
    vector_masks_from_extensions == native_masks ||
        error("vectorized formula extension mismatch")
    ...
    tree_predictions = ModalDecisionTrees.apply(
        fixture.tree, fixture.modalities; print_progress=false
    )
    ...
    return (
        rules=length(fixture.rules),
        instances=SoleData.ninstances(fixture.dataset),
        extension_cases=sum(length.(scalar_extensions)),
        predictions=length(tree_predictions),
        world_order=:dataset_frame,
    )
end
    ...
    for rule in fixture.rules
        ...
        for instance in 1:SoleData.ninstances(dataset)
            for world in SoleData.allworlds(SoleData.frame(dataset, instance))
                total += SoleData.check(
                    formula, dataset, instance, world; perform_normalization=false
                )
            end
        end
    end
...
function supported_cold(fixture)
    ...
    cold = SoleData.SupportedLogiset(
        SoleData.base(fixture.dataset);
        conditions=support.metaconditions,
        relations=relations,
        onestep_precompute_globmemoset=true,
        onestep_precompute_relmemoset=false,
    )
    return formula_check(fixture, cold)
end

function fresh_fixture(base, dataset; scalar_layer=base.bridge_scalar !== nothing)
    modalities = SoleData.MultiLogiset(dataset)
    scalar_state = DecisionListBatchAdapter.prepare(
        base.decision_list, modalities; vectorized=false
    )
    vector_state = DecisionListBatchAdapter.prepare(
        base.decision_list, modalities; vectorized=true
    )
    bridge_scalar = if scalar_layer
        build_bridge_scalar_layer_state(dataset, vector_state; vectorized=false)
    else
...
function churn_fixtures(base; count=6)
    return [
        fresh_fixture(base, make_supported_dataset(APPLY_NINSTANCES, APPLY_NPOINTS)) for
        _ in 1:count
    ]
end
...
function apply_mode(fixture, mode)
    ...
    mode === "deployed-modal-tree" && return ModalDecisionTrees.apply(
        fixture.tree, fixture.modalities; print_progress=false
    )
    mode === "decision-list-apply" &&
        return SoleModels.apply(fixture.decision_list, fixture.modalities)
    ...
    mode === "aletheia-data-bridge-scalar" &&
        return scalar_layer_apply(fixture.bridge_scalar, fixture.decision_list)
    mode === "aletheia-data-bridge-vectorized" &&
        return scalar_layer_apply(fixture.bridge_vectorized, fixture.decision_list)
    mode === "aletheia-data-scalar" &&
        return scalar_layer_apply(fixture.dense_scalar, fixture.decision_list)
    mode === "aletheia-data-vectorized" &&
        return scalar_layer_apply(fixture.dense_vectorized, fixture.decision_list)
    return error("unknown apply mode $mode")
end

...
function allocation_sample(f)
    ...
    return (
        allocs=Int(diff.malloc + diff.realloc + diff.poolalloc + diff.bigalloc),
        bytes=Int(diff.allocd),
    )
end
function emit(mode, step, m)
    return println("profile mode=$(mode) step=$(step) allocs=$(m.allocs) bytes=$(m.bytes)")
end
...
function profile_fresh_apply(mode, base)
    ...
    warm_profile_fixture = fresh_fixture(
        base, make_supported_dataset(APPLY_NINSTANCES, APPLY_NPOINTS)
    )
    Profile.Allocs.@profile sample_rate = 1 apply_mode(warm_profile_fixture, mode)
    Profile.Allocs.clear()
    fresh_fixture_value = fresh_fixture(
        base, make_supported_dataset(APPLY_NINSTANCES, APPLY_NPOINTS)
    )
    ...
    for line in split(text, '\n')
        source = strip(line)
        (
            occursin("deployed_apply", source) ||
            occursin("decisionlist", source) ||
            occursin("AletheiaData", source) ||
            occursin("Sole", source) ||
            occursin("scalar.jl", source) ||

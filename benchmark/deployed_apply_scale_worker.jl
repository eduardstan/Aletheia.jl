...
function emit(mode, phase, seed, ninstances, npoints, m)
    s = m.sample
    return println(
        "scale-result seed=$(seed) instances=$(ninstances) points=$(npoints) mode=$(mode) phase=$(phase) " *
...
for seed in APPLY_SEEDS
    ...
    println(
        "scale-parity=PASS seed=$(seed) instances=$(ninstances) points=$(npoints) " *
    ...
    for mode in ("decision-list-apply", "aletheia-scalar", "aletheia-vectorized")
        ...
        churn = [
            fresh_fixture(base, make_supported_dataset(ninstances, npoints)) for _ in 1:6
        ]
        cursor = Ref(0)

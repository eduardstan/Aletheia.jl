...
function timed_sample(f)
    ...
    return TimedSample(
        Float64(elapsed),
        Int(diff.malloc + diff.realloc + diff.poolalloc + diff.bigalloc),
        Int(diff.allocd),
    )
end
...
function measure(f; warm=true, samples=5)
    ...
    return (sample=sample, minimum=minimum(times), maximum=maximum(times), samples=samples)
end

function emit(mode, phase, measured; seed=missing)
    s = measured.sample
    return println(
        "result seed=$(seed) mode=$(mode) phase=$(phase) time_ns=$(s.time) allocs=$(s.allocs) bytes=$(s.bytes) " *
...
for seed in APPLY_SEEDS
    ...
    println(
        "parity=PASS seed=$(seed) rules=$(gate.rules) instances=$(gate.instances) " *
    ...
    elseif endswith(mode, "-construction")
        ...
        if startswith(mode, "aletheia-data")
            builder = if occursin("bridge", mode)
                build_bridge_scalar_layer_state
            else
            ...
            m = measure(; warm=false) do
                DecisionListBatchAdapter.prepare(
                    fixture.decision_list, fixture.modalities; vectorized=vectorized
                )
            end
        end
        emit(mode, "construction", m; seed=seed)
        ...
        unseen = fresh_fixture(
            fixture, make_supported_dataset(APPLY_NINSTANCES, APPLY_NPOINTS)
        )
        first_measurement = measure(() -> apply_mode(unseen, mode); warm=false)
        emit(
            mode,
            mode === "supported-cold" ? "cold-construction-first-use" : "first-use",
            first_measurement;
            seed=seed,
        )

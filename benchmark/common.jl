...
function parse_seed(text)
    ...
    return if startswith(lowercase(clean), "0x")
        parse(UInt64, clean[3:end]; base=16)
    else
        parse(UInt64, clean)
    end
end
...
const MODAL_SIGNATURE = Aletheia.Signature((
    Aletheia.NEGATION,
...
const MV_SIGNATURE = Aletheia.Signature((
...
function seeded_tree(
    depth, rng; shared=false, binary_ops=(:and, :or, :implies), counter=Ref(0)
)
    if depth == 0
        counter[] += 1
    ...
    if shared
        child = seeded_tree(
            depth - 1, rng; shared=true, binary_ops=binary_ops, counter=counter
        )
...
function seeded_unshared(depth, seed=SEED; binary_ops=(:and, :or, :implies))
    return seeded_tree(depth, MersenneTwister(seed); binary_ops=binary_ops)
end
function seeded_shared(depth, seed=SEED; binary_ops=(:and, :or, :implies))
    return seeded_tree(depth, MersenneTwister(seed); shared=true, binary_ops=binary_ops)
end
...
function unshared(depth, counter=Ref(0))
    ...
    return recipe(:and, unshared(depth - 1, counter), unshared(depth - 1, counter))
end

function recipe_atoms(r::Recipe)
    return r.op === :atom ? [r.atom] : vcat((recipe_atoms(c) for c in r.children)...)
end
function shared(depth)
    depth == 0 && return atomrecipe("p")
    child = shared(depth - 1)
    return recipe(:and, child, child)
end
...
function modal_formula(depth)
    ...
    return recipe(:or, modal_formula(depth - 1), atomrecipe("p"))
end
function random_recipe(rng, depth, counter=Ref(0); modal=true)
    ...
    if modal && rand(rng) < 0.30
        return recipe(
            rand(rng, (:diamond, :box)), random_recipe(rng, depth - 1, counter; modal=modal)
        )
    end
...
function build_a(r::Recipe, pool)
    ...
    return Aletheia.branch(pool, AOPS[r.op], (build_a(c, pool) for c in r.children)...)
end
function build_s(r::Recipe)
    r.op === :atom && return SoleLogics.Atom(r.atom)
    return SoleLogics.SyntaxBranch(SOPS[r.op], (build_s(c) for c in r.children)...)
end
...
function canonical(f::SoleLogics.SyntaxBranch)
    return (
        :branch,
        SoleLogics.syntaxstring(SoleLogics.token(f)),
        canonical.(SoleLogics.children(f)),
    )
end
...
function measure_samples(f; samples=BENCH_SAMPLES)
    ...
    return Measurement(paired.time, paired.allocs, paired.memory)
end
...
function _child_failure(exitcode, error_output; context="", output="")
    ...
    return Measurement(
        ...
        missing,
        missing,
        "$(prefix)failed (exit code $(exitcode)): $(message)",
        missing,
        :failed,
    )
end
...
function _run_child(command)
    output_path, output_io = mktemp()
    close(output_io)
    error_path, error_io = mktemp()
    close(error_io)
    ...
    rm(error_path; force=true)
    return process, output, error_output
end

function guarded_measure(
    label, kind, side, argument, f=nothing; timeout=CASE_TIMEOUT, seed=SEED
)
    println("[case] ", label)
    flush(stdout)
    project = @__DIR__
    ...
    process.exitcode != 0 &&
        return _child_failure(process.exitcode, error_output; output=output)
    values = try
        parse.(Float64, split(strip(output)))
    catch
        Float64[]
    end
    length(values) == 4 || return Measurement(
        missing, missing, missing, "failed (invalid measurement)", missing, :failed
    )
    return Measurement(
        values[1], Int(round(values[2])), Int(round(values[3])), "", values[4], :measured
    )
end
...
function section_measure(label, cases, side; timeout=CASE_TIMEOUT, seed=SEED, seeds=nothing)
    ...
    for i in eachindex(cases)
        for offset in 0:(length(seed_list) - 1)
            push!(
                order,
                (case_index=i, seed=seed_list[1 + mod(i - 1 + offset, length(seed_list))]),
            )
        end
    end
    project = @__DIR__
    ...
    encoded = join(
        (
            string(
                cases[item.case_index][1], "=", cases[item.case_index][2], "@", item.seed
            ) for item in order
        ),
        ";",
    )
    ...
    env = Dict{String,String}(ENV)
    env["BENCHMARK_SEED"] = string(seed_list[1])
    command = setenv(command, env)
    process, output, error_output = _run_child(command)
    ...
    if is_timeout_exitcode(process.exitcode)
        ...
        for line in lines
            ...
            parsed = try
                parse.(Float64, line[1:3])
            catch
                nothing
            end
            parsed === nothing && continue
            load = line[4] == "missing" ? missing : try
        ...
        if process.exitcode != 0 && length(result) == length(order)
            ...
            failure = _child_failure(
                process.exitcode, error_output; context="section", output=output
            )
            result = [failure for _ in order]
        elseif length(result) < length(order)
            failure = if process.exitcode == 0
                Measurement(
                    missing,
                    missing,
                    missing,
                    "failed (invalid or incomplete measurement)",
                    missing,
                    :failed,
                )
            else
        ...
        begin
            ...
            for i in eachindex(cases)
                match_index = findfirst(
                    k -> order[k].case_index == i && order[k].seed == seed_list[j],
                    eachindex(order),
                )
...
function fmt_time(x)
    ...
    return if x < 1_000
        @sprintf("%.1f ns", x)
    elseif x < 1_000_000
        @sprintf("%.2f μs", x / 1_000)
    else
        @sprintf("%.2f ms", x / 1_000_000)
    end
end
function fmt_measure(m)
    ...
    return if m.time === missing
        "empty"
    elseif isempty(m.note)
        fmt_time(m.time)
    else
        "$(fmt_time(m.time)) [$(m.note)]"
    end
end
function fmt_alloc(m::Measurement)
    return m.allocs === missing ? "—" : "$(m.allocs) / $(Base.format_bytes(m.memory))"
end

const rows = NamedTuple[]
function aggregate_measurements(measurements)
    isempty(measurements) &&
        return Measurement(missing, missing, missing, "no measurements", missing, :failed)
    all(is_measured, measurements) || begin
        ...
        return Measurement(
            ...
            missing,
            missing,
            something(summary, "no measurements"),
            missing,
            status,
        )
    end
    ...
    return Measurement(
        median(getfield.(measurements, :time)),
        isempty(allocs) ? missing : round(Int, median(allocs)),
...
function finite_values(measurements, field=:time)
    return [
        getfield(m, field) for
        m in measurements if is_measured(m) && getfield(m, field) !== missing
    ]
end
function stats(values)
    return if isempty(values)
        (median=missing, mean=missing, std=missing)
    else
        (
            median=median(values),
            mean=mean(values),
            std=length(values) < 2 ? 0.0 : std(values),
        )
    end
end
function time_summary(measurements)
    ...
    values = finite_values(measurements)
    s = stats(values)
    return if s.mean === missing
        "empty"
    else
        "$(fmt_time(s.median)) (mean $(fmt_time(s.mean)) ± $(fmt_time(s.std)))"
    end
end
function ratio_summary(row)
    s = stats(row.ratios)
    s.mean === missing && return "—"
    marker = s.mean - s.std <= 1.0 <= s.mean + s.std ? " [no clear winner]" : ""
    count_note = if length(row.ratios) == length(row.incumbent_seeds)
        ""
    else
        " [$(length(row.ratios))/$(length(row.incumbent_seeds)) seeds]"
    end
    @sprintf(
        "%.2fx (mean %.2fx ± %.2fx, range %.2f-%.2fx)%s%s",
        s.median,
        s.mean,
        s.std,
        minimum(row.ratios),
...
function addrow!(suite, incumbents, aletheias; allocations=true, note="")
    inc = aggregate_measurements(incumbents)
    ale = aggregate_measurements(aletheias)
    ratios = if all(is_measured, incumbents) && all(is_measured, aletheias)
        [i.time / a.time for (i, a) in zip(incumbents, aletheias)]
    else
        Float64[]
    end
    rs = stats(ratios)
    return push!(
        ...
        (
            ...
            note=note,
            incumbent_seeds=incumbents,
            aletheia_seeds=aletheias,
        ),
    )
end
function print_report()
    println(
        "suite | SoleLogics median (mean ± std) | Aletheia median (mean ± std) | ratio median (mean ± std) | allocations (Sole/Aletheia) | note",
    )
    println(
        "------|---------------------------------|--------------------------------|----------------------------|--------------------------|-----",
    )
    for row in rows
        alloc =
            if row.allocations &&
               row.incumbent.status === :measured &&
               row.aletheia.status === :measured
                "$(fmt_alloc(row.incumbent)) ; $(fmt_alloc(row.aletheia))"
            else
                "—/—"
            end
        println(
            "$(row.suite) | $(time_summary(row.incumbent_seeds)) | $(time_summary(row.aletheia_seeds)) | $(ratio_summary(row)) | $alloc | $(row.note)",
        )
    end
end
...
function edge_data(n, density, seed)
    ...
    return [(i, j) for i in 1:n for j in 1:n if rand(rng) < density]
end
function seeded_sets(names, n, seed=SEED)
    rng = MersenneTwister(seed)
    return Dict(name => Set(w for w in 1:n if rand(rng, Bool)) for name in unique(names))
end
function a_boolean_model(n, edges; sets=nothing)
    ...
    sets === nothing && (
        sets = Dict(
            "p" => Set(w for w in worlds_a if isodd(w)),
            "q" => Set(w for w in worlds_a if mod(w, 3) == 0),
        )
    )
    return Aletheia.Model(frame, Aletheia.BOOLEAN, sets)
end
function s_boolean_model(n, edges; sets=nothing)
    worlds_s = SoleLogics.World.(1:n)
    graph = SoleLogics.Graphs.SimpleDiGraph(n)
    for (i, j) in edges
        SoleLogics.Graphs.add_edge!(graph, i, j)
    end
    frame = SoleLogics.SimpleModalFrame(worlds_s, graph)
    sets === nothing && (
        sets = Dict(
            "p" => Set(w for w in 1:n if isodd(w)),
            "q" => Set(w for w in 1:n if mod(w, 3) == 0),
        )
    )
    atom_names = collect(keys(sets))
    assignment = Dict(
        w => SoleLogics.TruthDict(
            Dict(atom => (w.name in sets[atom]) for atom in atom_names)
        ) for w in worlds_s
    )
    return SoleLogics.KripkeStructure(frame, assignment)
end
...
function interval_adjacency_a(frame, relation_name, frame_worlds)
    ...
    return Aletheia._relation_adjacency(frame, relation_name, positions)
end
...
function interval_subset_a(frame, relation_set, frame_worlds)
    return sum(
        length(collect(Aletheia.accessible(frame, source, relation))) for
        relation in relation_set for source in frame_worlds
    )
end
function interval_subset_s(frame, relation_set, frame_worlds)
    return sum(
        length(collect(SoleLogics.accessibles(frame, source, relation))) for
        relation in relation_set for source in frame_worlds
    )
end
function interval_check_a_setup(n)
    ...
    return frame, frame_worlds, formula, valuation
end
function interval_check_s_setup(n)
    ...
    p = SoleLogics.Atom("p")
    formula = SoleLogics.SyntaxBranch(
        SoleLogics.DiamondRelationalConnective(SoleLogics.IA_L), p
    )
...
function finite_truth(index)
    return SoleLogics.ManyValuedLogics.FiniteTruth(index)
end
...
function mv_setup(side, algebra_name, depth)
    ...
    algebra_name in ("godel", "lukasiewicz", "h4") ||
        error("unknown finite algebra $algebra_name")
    if side == "aletheia"
        pool = mv_pool_a()
        f = build_a(r, pool)
        algebra = if algebra_name == "godel"
            Aletheia.G3
        elseif algebra_name == "lukasiewicz"
            Aletheia.Ł3
        else
            Aletheia.H4
        end
        ...
        values = Dict(
            (name, 1) => finite_truth_value(1 + mod(i, n)) for
            (i, name) in enumerate(recipe_atoms(r))
        )
    ...
    n = algebra_name == "h4" ? 4 : 3
    values = Dict(
        name => finite_truth(1 + mod(i, n)) for (i, name) in enumerate(recipe_atoms(r))
    )
    alg = if algebra_name == "godel"
        SoleLogics.ManyValuedLogics.G3
    elseif algebra_name == "lukasiewicz"
        SoleLogics.ManyValuedLogics.Ł3
    else
        SoleLogics.ManyValuedLogics.H4
    end
    return f, SoleLogics.TruthDict(values), alg
end
...
function score_a(hypotheses, examples)
    return sum(
        Aletheia.check(h, example.interpretation, 1) == example.positive for h in
                                                                             hypotheses for
        example in examples
    )
end
function score_s(hypotheses, examples)
    return sum(
        SoleLogics.check(h, example[1], example[2]; perform_normalization=false) ==
        example[3] for h in hypotheses for example in examples
    )
end
...
function contraction_model(n, q, seed=SEED)
    ...
    labels = Dict(
        atom => Set(
            w for (position, w) in enumerate(permutation) if
            ((div(position - 1, max(1, n ÷ q)) >> (i - 1)) & 1) == 1
        ) for (i, atom) in enumerate(atoms)
    )
    adjacency = Dict(w => collect(1:n) for w in 1:n)
    model = Aletheia.Model(
        Aletheia.Frame(Tuple(1:n), Dict(:R => adjacency); index=true),
        Aletheia.BOOLEAN,
        labels,
    )
    return model, atoms
...
function contraction_formulas(pool, atoms, count)
    ...
    formulas = Aletheia.Formula[
        p,
        Aletheia.branch(pool, Aletheia.Box(:R), p),
    ...
    while length(formulas) < count
        ...
        push!(
            formulas,
            if isodd(i)
                Aletheia.branch(pool, Aletheia.Box(:R), formulas[end])
            else
...
function parse_ratio_measurements(measurements)
    ...
    return Measurement(
        median(getfield.(measurements, :time)),
        round(Int, median(getfield.(measurements, :allocs))),
        round(Int, median(getfield.(measurements, :memory))),
        "",
        missing,
        :measured,
    )
...
function external_measure(code; reps=DEEP ? 2 : 1)
    ...
    for _ in 1:reps
        ...
        elseif process.exitcode != 0
            failure = _child_failure(
                process.exitcode, error_output; context="cold child", output=output
            )
    ...
    return if isempty(values)
        (status=:failed, note="failed (no measurements)")
    else

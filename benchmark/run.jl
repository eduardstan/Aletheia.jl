...
println(
    "samples: ",
    BENCH_SAMPLES,
    "; fixed-sample timing (legacy budget: ",
    BENCH_SECONDS,
    " s); hard per-call timeout: ",
    CASE_TIMEOUT,
    " s",
)
println()
...
for depth in (2, 4, 6)
    ...
    a = build_a(r, ap)
    s = build_s(r)
    am = a_boolean_model(1, Tuple{Int,Int}[])
    sm = s_boolean_model(1, Tuple{Int,Int}[])
    @assert Aletheia.check(a, am, 1) == SoleLogics.check(
        s,
        sm,
        first(SoleLogics.allworlds(SoleLogics.frame(sm)));
...
for seed in SEEDS
    ...
    for trial in 1:(DEEP ? 192 : 96)
        n = rand(rng_gate, 2:10)
        density = rand(rng_gate)
        edges = edge_data(n, density, rand(rng_gate, 1:typemax(Int)))
        sets = Dict("p$(i)" => Set(w for w in 1:n if rand(rng_gate, Bool)) for i in 1:6)
        model = a_boolean_model(n, edges; sets=sets)
        p = modal_pool_a()
        quotient = Aletheia.bisimulation_contraction(
            model; atoms=collect(keys(sets)), relations=[:R]
        )
        ...
        for _ in 1:(DEEP ? 24 : 16)
            ...
            for world in Aletheia.worlds(Aletheia.frame(model))
                @assert Aletheia.check(formula, model, world) == Aletheia.check(
                    formula, qmodel, Aletheia.contraction_world(quotient, world)
                )
            end
        end
    end
...
syntax_cases = Tuple{String,String}[];
syntax_labels = String[];
for depth in (DEEP ? (3, 6, 9) : (2,))
    for label in ("unshared", "shared")
        push!(syntax_cases, ("construction", "$label:$depth"))
        push!(syntax_labels, "construction/$label depth=$depth")
    end
    for (kind, name) in
        (("parsing", "parsing"), ("printing", "printing"), ("roundtrip", "round-trip"))
        push!(syntax_cases, (kind, "$depth"))
        push!(syntax_labels, "$name depth=$depth")
    end
end
for n in (DEEP ? (16, 128, 512) : (16,))
    push!(syntax_cases, ("equality", "$n"))
    push!(syntax_labels, "equality isequal chain=$n")
end
...
add_section_rows!(
    "propositional check",
    ["propositional check depth=$(arg)" for (_, arg) in prop_cases],
    prop_cases,
)
ext_cases = [
    ("prop_extension", "$n:$depth") for
    (n, depth) in (DEEP ? ((8, 3), (32, 4), (128, 5), (256, 6)) : ((8, 3), (32, 4)))
]
add_section_rows!(
    "extension",
    [
        "extension finite model worlds=$(split(arg, ':')[1]) depth=$(split(arg, ':')[2])"
        for (_, arg) in ext_cases
    ],
    ext_cases;
...
modal_cases = [
    ("modal_check", "$n:$density:$depth") for n in (DEEP ? (8, 24, 64) : (8, 24)),
    density in (0.15, 0.50), depth in (DEEP ? (2, 4, 6) : (2, 4))
]
add_section_rows!(
    "random modal check",
    [
        "random modal check worlds=$(split(arg, ':')[1]) density=$(split(arg, ':')[2]) depth=$(split(arg, ':')[3])"
        for (_, arg) in modal_cases
    ],
    modal_cases;
...
interval_cases = Tuple{String,String}[];
interval_labels = String[];
for n in (DEEP ? (6, 12, 24, 36) : (6,))
    push!(interval_cases, ("interval_adjacency", string(n)))
    push!(interval_labels, "interval adjacency n=$n")
    push!(interval_cases, ("interval_check", string(n)))
    push!(interval_labels, "Allen BEFORE check n=$n")
end
for name in ("ia3", "ia7", "rcc5")
    push!(interval_cases, ("interval_subset", "$name:6"))
    push!(interval_labels, "interval subset $name n=6")
end
add_section_rows!(
    "interval / dimensional",
    interval_labels,
    interval_cases;
...
mv_cases = [
    ("many_check", "$algebra_name:$depth") for
    algebra_name in ("godel", "lukasiewicz", "h4"), depth in (DEEP ? (2, 4, 6) : (2,))
]
mv_labels = String[]
for (_, arg) in mv_cases
    algebra, depth = split(arg, ':')
    push!(
        mv_labels,
        "$(algebra == "h4" ? "non-chain H4" : "finite chain $algebra") value depth=$depth",
    )
end
add_section_rows!(
    "many-valued",
    mv_labels,
    mv_cases;
    note="finite designated check; H4 is the landed non-chain FLew algebra",
)

println("[learning from interpretations]")
ilp_cases = [
    ("ilp_score", "$model_count:$hypothesis_count") for
    (model_count, hypothesis_count) in (DEEP ? ((8, 4), (32, 12), (64, 24)) : ((8, 4),))
]
add_section_rows!(
    "ILP interpretation scoring",
    [
        "ILP interpretation scoring models=$(split(arg, ':')[1]) hypotheses=$(split(arg, ':')[2])"
        for (_, arg) in ilp_cases
    ],
    ilp_cases;
...
println(
    "n | quotient | ratio | C | P_orig | P_quot | K* | measured crossover (K: orig ms / quotient ms)",
)
contraction_cases = Tuple{String,String}[];
contraction_ranges = NamedTuple[];
contraction_models = DEEP ? (96, 192, 384) : (48,);
contraction_counts = DEEP ? 5 : 2;
contraction_curve = DEEP ? (1, 2, 4, 8, 16, 32, 64) : (1, 8, 32)
for n in contraction_models, q in (DEEP ? (1, 4, 16, n) : (1, n))
    q > n && continue
    cindex = length(contraction_cases) + 1
    push!(contraction_cases, ("contraction_cost", "$n:$q"))
    orig_indices = Int[]
    quot_indices = Int[]
    curve_orig = Int[]
    curve_quot = Int[]
    for i in 1:contraction_counts
        push!(contraction_cases, ("contraction_orig", "$n:$q:$i:8"))
        push!(orig_indices, length(contraction_cases))
    end
    for i in 1:contraction_counts
        push!(contraction_cases, ("contraction_quot", "$n:$q:$i:8"))
        push!(quot_indices, length(contraction_cases))
    end
    for k in contraction_curve
        push!(contraction_cases, ("contraction_batch_orig", "$n:$q:8:$k"))
        push!(curve_orig, length(contraction_cases))
        push!(contraction_cases, ("contraction_batch_quot", "$n:$q:8:$k"))
        push!(curve_quot, length(contraction_cases))
    end
    push!(
        contraction_ranges,
        (
            ...
            orig=orig_indices,
            quot=quot_indices,
            curve_orig=curve_orig,
            curve_quot=curve_quot,
        ),
    )
end
contraction_measurements = section_measure(
    "contraction", contraction_cases, "aletheia"; timeout=DEEP ? 180 : 120, seeds=SEEDS
)
...
for entry in contraction_ranges
    ...
    c = aggregate_measurements([r.c for r in seed_records])
    po = aggregate_measurements([r.po for r in seed_records])
    pq = aggregate_measurements([r.pq for r in seed_records])
    ...
    println(
        "$(entry.n) | $(entry.q) | $(@sprintf("%.3f", entry.q / entry.n)) | $(time_summary([r.c for r in seed_records])) | $(time_summary([r.po for r in seed_records])) | $(time_summary([r.pq for r in seed_records])) | $ktext | ",
    )
    push!(
        contraction_records,
        (
...
function cold_measure(x, i)
    return if x.status === :measured
        Measurement(x.values[i] * 1e6, missing, missing, "", missing, :measured)
    else
...
addrow!(
    "cold package load",
    [cold_measure(x[2], 1) for x in load_values],
    [cold_measure(x[1], 1) for x in load_values];
    allocations=false,
)
addrow!(
    "cold time-to-first-result",
    [cold_measure(x[2], 2) for x in load_values],
    [cold_measure(x[1], 2) for x in load_values];
    allocations=false,
)
...
println();
print_report();
println("benchmark wall clock: ", @sprintf("%.1f s", wall_clock));
...
for row in rows
    append!(
        recorded_seed_loads,
        [
            measurement.load for measurement in row.incumbent_seeds if
            measurement.load !== missing && isfinite(measurement.load)
        ],
    )
    append!(
        recorded_seed_loads,
        [
            measurement.load for measurement in row.aletheia_seeds if
            measurement.load !== missing && isfinite(measurement.load)
        ],
    )
end
for measurements in contraction_measurements
    append!(
        recorded_seed_loads,
        [
            measurement.load for measurement in measurements if
            measurement.load !== missing && isfinite(measurement.load)
        ],
    )
end
load_verdict = benchmark_load_verdict(
...
load_marker = if load_verdict.publishable
    ""
elseif ALLOW_CONTENDED
...
failure_marker = if run_has_failures
...
open(artifact, "w") do io
    ...
    println(
        io, "failure_publishability=$(run_has_failures ? "non-publishable" : "publishable")"
    )
    ...
    for row in rows
        ...
        println(
            ...
            row.suite,
            " | ",
            time_summary(row.incumbent_seeds),
            " | ",
            time_summary(row.aletheia_seeds),
            " | ",
            ratio_summary(row),
            " | ",
            if row.allocations &&
            ...
            " | samples=",
            samples,
            " per seed; seed_ratios=",
            join([@sprintf("%.6f", x) for x in row.ratios], ","),
            "; seed_outcomes=",
            join(
                [
                    "$(i.status)/$(a.status)" for
                    (i, a) in zip(row.incumbent_seeds, row.aletheia_seeds)
                ],
                ",",
            ),
            "; seed_notes=",
            ...
            join(
                [
                    "$(i.load === missing ? "missing" : @sprintf("%.2f", i.load))/$(a.load === missing ? "missing" : @sprintf("%.2f", a.load))"
    ...
    for (entry, record) in zip(contraction_ranges, contraction_records)
        ...
        for (k, oi, qi) in zip(contraction_curve, entry.curve_orig, entry.curve_quot)
            ...
            println(
                io,
                "contraction_batch n=$(record.n) q=$(record.q) K=$k | original=$(time_summary(bos)) | quotient_total=$(time_summary(totals)) | samples=2000 per seed",
            )
        end
    end
end

...
const APPLY_DEPTH = 5
for (name, path) in (
    ("SOLELOGICS_PATH", SOLELOGICS_PATH),
    ("SOLEDATA_PATH", SOLEDATA_PATH),
...
function load_value(line)
    ...
    return m === nothing ? missing : parse(Float64, m.captures[1])
end
...
sections = (
    ...
    "deployed-modal-tree",
    "decision-list-apply",
    "aletheia-scalar-construction",
    "aletheia-scalar",
    "aletheia-vectorized-construction",
    "aletheia-vectorized",
    "aletheia-data-bridge-scalar-construction",
    "aletheia-data-bridge-scalar",
    "aletheia-data-bridge-vectorized-construction",
    "aletheia-data-bridge-vectorized",
    "aletheia-data-scalar-construction",
    "aletheia-data-scalar",
    "aletheia-data-vectorized-construction",
    "aletheia-data-vectorized",
)
records = String[]
errors = String[]
...
try
    ...
    for path in
        (ROOT, SOLELOGICS_PATH, SOLEDATA_PATH, SOLEMODELS_PATH, MODALDECISIONTREES_PATH)
    ...
    open(result_path, "w") do io
        ...
        println(
            io, "julia_num_precompile_tasks=$(get(ENV, "JULIA_NUM_PRECOMPILE_TASKS", ""))"
        )
        ...
        println(
            io,
            "seeds=0xA1E7_2024,0x5EED_2025,0xC0FF_EE42,0x1234_5678,0x9ABC_DEF0 data_seed=$(APPLY_DATA_SEED) train_seed_default=$(APPLY_TRAIN_SEED)",
        )
        println(
            io,
            "workload=instances=$(APPLY_NINSTANCES),points=$(APPLY_NPOINTS),depth=$(APPLY_DEPTH)",
        )
        println(
            io,
            "scale_cases=32:8,64:8,128:8,256:8,512:8; scale_modes=decision-list-apply,aletheia-scalar,aletheia-vectorized; scale_timeout_s=900; scale_samples=3; scale_address_space_kb=6000000",
        )
        println(
            io,
            "profile=Profile.Allocs sample_rate=1 on one never-used fresh-dataset churn apply; vectorized callback versus native decision-list apply",
        )
        println(
            io,
            "sole_controls=full_memo=true one_step_memo=true global_precompute=true relational_precompute=false",
        )
        println(
            io,
            "sole_paths=formula-check=SoleData.check; modal-tree=ModalDecisionTrees.apply/modalstep/checkcondition; decision-list=SoleModels.apply/SoleLogics.check",
        )
        println(
            io,
            "aletheia_path=DecisionListBatchAdapter prepared model family; callback scalar/vectorized and DenseFeatureStore scalar/vectorized paths; conversion outside apply timing",
        )
        for package_name in
            ("Aletheia", "SoleLogics", "SoleData", "SoleModels", "ModalDecisionTrees")
            dependency = first(
                filter(dep -> dep.name == package_name, collect(values(Pkg.dependencies())))
            )
    ...
    differential_out, differential_out_io = mktemp(ROOT)
    close(differential_out_io)
    differential_error, differential_error_io = mktemp(ROOT)
    close(differential_error_io)
    julia = Base.julia_cmd()
    differential_command = `$(julia) --startup-file=no --project=$environment $(joinpath(@__DIR__, "deployed_apply_differential.jl"))`
    differential_process = run(
        pipeline(differential_command; stdout=differential_out, stderr=differential_error);
        wait=false,
    )
    ...
    rm(differential_out; force=true)
    rm(differential_error; force=true)
    ...
    for section in sections
        ...
        outpath, outio = mktemp(ROOT)
        close(outio)
        errpath, errio = mktemp(ROOT)
        close(errio)
    ...
    for (ninstances, npoints) in ((32, 8), (64, 8), (128, 8), (256, 8), (512, 8))
        ...
        push!(
            records,
            "scale-section=instances=$(ninstances),points=$(npoints) maxrss_kb=$(maxrss) uptime_before=$(before) uptime_after=$(after)",
        )
        scale_lines = [
            strip(line) for
            line in split(output, '\n') if startswith(strip(line), "scale-result ") ||
            startswith(strip(line), "scale-parity=")
        ]
        if process.exitcode == 124
            push!(
                records,
                "scale-skip=instances=$(ninstances),points=$(npoints),reason=timeout maxrss_kb=$(maxrss) uptime_before=$(before) uptime_after=$(after)",
            )
        elseif process.exitcode in (137, 9) ||
               occursin("Killed process", stderr) ||
               occursin("Cannot allocate memory", stderr) ||
               occursin("Resource temporarily unavailable", stderr)
            push!(
                records,
                "scale-skip=instances=$(ninstances),points=$(npoints),reason=memory maxrss_kb=$(maxrss) uptime_before=$(before) uptime_after=$(after)",
            )
        elseif process.exitcode != 0
            push!(
                errors,
                "scale section instances=$(ninstances),points=$(npoints) exit=$(process.exitcode): $(strip(stderr))",
            )
        elseif isempty(scale_lines)
            push!(
                errors,
                "scale section instances=$(ninstances),points=$(npoints) emitted no result",
            )
        else
            append!(
                records,
                [
                    "$line uptime_before=$(before) uptime_after=$(after)" for
                    line in scale_lines
                ],
            )
        end
    ...
    profile_process = run(
        pipeline(profile_command; stdout=outpath, stderr=errpath); wait=false
    )
    ...
    profile_lines = [
        strip(line) for
        line in split(profile_output, '\n') if startswith(strip(line), "profile ") ||
        startswith(strip(line), "profile-site ") ||
        startswith(strip(line), "profile-site-source ") ||
        startswith(strip(line), "profile-top-source ") ||
        startswith(strip(line), "profile-cold ") ||
        startswith(strip(line), "profile-note=")
    ]
    profile_process.exitcode == 0 || push!(
        errors,
        "profile section exit=$(profile_process.exitcode): $(strip(profile_stderr))",
    )
    isempty(profile_lines) && push!(errors, "profile section emitted no attribution")
    append!(
        records,
        ["$line uptime_before=$(before) uptime_after=$(after)" for line in profile_lines],
    )
finally
...
load_gate_pass =
    ...
    peak_load !== missing &&
    peak_load <= 7.175 &&
    end_load - start_load <= 2.56
load_gate_pass ||
    push!(errors, "load gate failed: start=$(start_load) end=$(end_load) peak=$(peak_load)")

open(result_path, "a") do io
    println(io, "uptime_after_run=$(run_end_uptime)")
    println(
        io,
        "load_gate=$(load_gate_pass ? "PASS" : "FAIL") start=$(start_load) end=$(end_load) peak=$(peak_load)",
    )
    if isempty(errors)

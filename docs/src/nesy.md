...
`ske_roundtrip` enumerates a declared finite case set, validates and algebra-specifically
canonicalizes outputs, extracts an exact `RuleArtifact` or `TreeArtifact`, verifies the
artifact against the callable,

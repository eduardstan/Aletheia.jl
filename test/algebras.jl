@testset "finite FLew algebras" begin
    ...
    for (a, expected_join, expected_lattice_meet, expected_fusion) in named
        ...
        @test a.fusion == UInt8.(expected_fusion)
        @test truth_type(a) === UInt8 && carrier(a) == domain(a)
        ...
        for x in domain(a), y in domain(a), z in domain(a)
            @test implication(a, x, y) == a.implication[Int(x), Int(y)]
            @test (precedeq(a, fusion(a, x, y), z) == precedeq(a, x, implication(a, y, z)))
        end
    end

...
- Stage-2a SoleModels consumer routing remains benchmark-only: `benchmark/dataset_consumer.jl` builds disposable package copies; repeated distributions, load records, parity/loss rows, and baseline-cold allocation nondeterminism live in `data/al-dataset-consumer/report.md`, `run.txt`, and `repetitions/`.

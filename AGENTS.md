...
- The native callable-connective methods at the end of `src/syntax.jl` and the migration ones in `src/compatibility.jl` are a matched pair: the compatibility methods take the wider `Formula`, so the native ones must repeat the same connective unions with `Atom`/`Branch` operands or they become ambiguous rather than more specific.

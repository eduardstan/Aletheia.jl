...
- `inverse` is contractually the converse; a relation whose converse this vocabulary does not name throws. `MINIMUM` and `MAXIMUM` are the only such values. `test/relation_properties.jl` holds the generated-input relation laws (converse, involution, JEPD, fast path versus predicate, IA3/IA7 unions) and picks up newly exported relation values automatically.
